...
- Numeric/locale-agnostic tokens (`"2x"`, `"12px"`, version strings, `CraftStation`) stay plain strings.
...
Keep literal in every locale: `CraftStation`, `.craftstation/worktrees`, `WSL`, `Dev Drive`, `Linux`, provider names (`Claude`, `Codex`, `Gemini`).
