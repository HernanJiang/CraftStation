...
The preload bridge (`window.craftstation`) wraps all IPC into typed async methods defined by the `CraftStationBridge` type, declared in `src/shared/ipc/bridge.ts` and re-exported via the `src/shared/ipc.ts` barrel.
...
Worktree paths are computed within a centralized directory (`~/.craftstation/worktrees/<repo-id>/<branch-id>`) via `src/supervisor/git.ts` and `src/shared/worktree.ts`.
