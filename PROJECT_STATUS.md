...
- 并行 Feature：`v1.1.0 — Compatibility Bridge & Model × Harness Composition`。worktree `.worktrees/v1.1.0-compatibility-bridge`，分支 `dev/v1.1.0-compatibility-bridge`。Status：**`RE-PLANNED / CODER v1.1.3 AUTHORIZED`**（`eaf2d35`）。原裁决 FAIL/BLOCKED（`debugger_1.1.2.md`）；re-plan 采纳 Debugger 给定的唯一重受理路径：真实 CPA release 二进制 + 独立 Compatibility Adapter + OpenCode fixture-scoped 跨组合真实 Agent Loop + CPA credential namespace 账号绑定/secret-free 持久化。
- 并行 Feature：`v1.2.0 — Unified MCP + Skills Capability Foundation`。worktree `.worktrees/v1.2.0-mcp-skills-capability`，分支 `dev/v1.2.0-mcp-skills-capability`。Status：**`DEBUGGER RE-REVIEW PASS (DEV)`**（`e8f3ece`，候选 `de052d3`）。F1–F8 全部关闭；typecheck/lint/build 全绿；全量 vitest 残留仅 2 个 jsdom 负载 flaky（单跑通过）；真实 Electron 应用启动 smoke PASS（ABI 根因修复后 CDP 可达，0 运行时错误）。发布版本号 `1.7.0`。`report_1.2.md` 授权生成；merge/tag/push 仍需用户授权。
