...
- **`setPendingSteer` 超时导致整渲染器崩溃修复**：用户真机崩溃报告（v1.2.16）：`unhandled-rejection: Error invoking remote method 'craftstation:set-pending-steer': Supervisor request "setPendingSteer" timed out.` 根因：`ThreadComposerSection` 的队列「立即发送」`void sendQueuedFollowUpNow(thread)` 无 catch；working 线程走 `setThreadPendingSteer` → bridge invoke 被拒 → 未处理 rejection 触发崩溃页。修复：①`sendQueuedFollowUpNow` 失败时把已取出的排队消息放回队列（对齐 `flushQueuedFollowUp` 模式，用户文本不丢）并继续抛出；②调用点补 `.catch` → console.error + `toast.danger(friendlyError)`，supervisor 超时从此只是 toast 而不是整页崩溃。
- **新线程不遵守「默认权限 = 完全访问权限」修复（全 Harness）**：根因在 `resolveUnrestrictedPermissionConfig`（`src/shared/agents/unrestrictedPermissions.ts`）——通用猜测列表 `["bypassPermissions","yolo","never","dontAsk"]` 优先于 provider 自己声明的 `bypassPermissions` 姿态，导致 Kimi（≥0.42 起 `yolo` 只是 Ask When Needed，真正 Never Ask 是 `auto`）被解析成 `yolo`、Claude 被解析成 `bypassPermissions` 而非其声明的 `auto`；线程实际仍会要求批准，且 composer 权限 chip（fullAccessPolicyId = 声明的 bypass id）显示「请求批准」。修复：provider 声明的 bypass id（已广告时）优先，通用列表降级为兜底。Codex/Gemini/Copilot/Qwen/Pi/Cursor（never）、OpenCode/Muse/DeepSeek/CommandCode/Antigravity/Devin（yolo）、Qoder/Grok（bypassPermissions）、Factory（auto-high）结果不变。
- 生效面：草稿 UI（设置 → 一般 → 默认权限）、`createAppThread`（create_thread / spawn_peer / 计划任务）全走同一解析；Kimi 新开线程现在拿到 `approvalPolicy: "auto"` → 启动带 `--auto`（真 Never Ask），chip 显示「完全访问权限」。已存在的线程配置不变，可在 composer 手动切。
- 验证：新增 `unrestrictedPermissions.test.ts` 8 例全过；`registry.test.ts` 子代理姿态期望更新（claude/kimi → auto）；crossagentMcp + registry 202 例、kimi/claude/shared agents 447 例全过；typecheck PASS；触碰文件 oxlint 0 警告。
- **Grok 上下文容量显示 258K 而非 500K 修复**：归因——grok CLI 的 ACP `_meta.context_window` 是其 **auto-compact 触发阈值**（grok 自带文档 11-custom-models.md：`context_window` "tells Grok when to trigger auto-compaction"；状态栏文档另列 `auto_compact_threshold_percent`；二进制证实服务器经 `x-grok-context-window` 响应头动态上调窗口、下调被忽略），不是模型窗口。实测 grok-4.6 线程占用 292K 已超过上报的 258400 仍继续运行（随后才被 compact 到 6K），且 CLI 1.0.34 实测广告 `totalContextTokens: 500000`。渲染层 `preferAdvertisedContextWindow` 此前只在上报值是 262144/256000/128000 占位值时才让位于广告窗口，258400 不在其列。修复（`threadContextUsage.ts`）：①grok 线程上报值小于广告窗口时让位于广告窗口；②通用不可能态——占用 > 上报窗口且广告窗口更大时让位（真实窗口永不可能小于当前占用）。Codex 的 95% 紧凑限制（258400/272000 近邻比）两条规则都不触发，显示保持原样；Claude「用户选 1m 但 provider 实报 200K」的上报优先契约也不受影响。composer 上下文弹窗与状态栏共用 `resolveThreadContextUsageSummary`，一处修复两处生效。验证：`threadContextUsage.test.ts` 新增 3 例（grok 高占用/compact 后不振荡、codex 近邻保持、通用超限让位），文件 18 例全过；typecheck PASS；触碰文件 oxlint 0 警告。
- **Grok 表格不渲染修复（我们的问题）**：用户截图实锤 Grok 输出空表头表格（`| | |` 表头 + `|---|` 1 列分隔 + 2 列数据行）被渲染成原始竖线文本。根因：`normalizeGfmTableSeparators` 的 `isPotentialTableRow`（`src/renderer/components/thread/ChatPane/parts/items/ItemMarkdown.tsx`）要求行内含非 `|`/`:`/`-`/空白字符，空表头行被跳过 → 分隔行列数得不到修复 → remark-gfm 拒绝 → 纯文本。修复：放宽判定——行内 `|`≥2 且仅由 `|`/空白构成也视为表格行（不含 `-` 故不会误匹配分隔行）。验证：`ItemMarkdown.test.ts` 加 2 例、`ItemMarkdownInner.test.tsx` 加渲染级用例（复刻 Grok 真实形态），56/56 全过。
- **Kimi 额度耗尽不换号修复**：真机实验（`KIMI_CODE_HOME=<Jinwen profile> kimi -p ... --output-format stream-json`）实锤 5h 窗口耗尽返回 `provider.auth_error: 403 You've reached your 5-hour usage limit. Your quota will reset when the current 5-hour window ends...`——是 **403 + auth_error**，不是 402。根因：`isKimiPoolQuotaError`（`src/supervisor/agents/acp/sessionErrors.ts`）一刀切排除 401/403 → 5h 窗口耗尽永不识别 → 不标 exhausted、不 failover、下个 turn 又撞死账号。修复：403 按文案拆分——`KIMI_QUOTA_WINDOW_RE`（`usage limit` / `quota will reset` / `quota exceeded|exhausted` / `额度耗尽` / `额度不足` / `套餐已用完`）命中则配额错误（failover），否则（forbidden 等真鉴权失败）仍 fail-closed；httpStatus=403 与 message 含 `\b403\b` 两条路径同走该拆分，末尾通用正则补 `usage limit`。验证：`session.test.ts` 加用例（真实 verbatim 文案 + 反面 403 forbidden），134/134 全过。Grok/Claude 等渠道的 failover 机制（POOL_FAILOVER_PROVIDERS、6h TTL 恢复）本就健全，无需改动。
- **侧边栏 group 折叠按钮**：`SidebarProjectHeader.tsx` suffix 最左新增 × 折叠按钮（lucide `X` + 现有 `quickButtonClass`），仅展开态渲染，stopPropagation 后 `setProjectCollapsed(project.id, true)`；i18n msgid `Collapse {0}`，zh-CN「折叠 {0}」（已 extract）。验证：`SidebarProjectHeader.test.tsx` 加 2 例（点击折叠不触发 openNewThread、折叠态不渲染），8/8 全过。
- **agy 回合后进程退出调查结论（无需改代码）**：`.scratch/agy-exit-probe.mjs` 真机探针实证——**成功回合后 agy 存活**（75s 观察无退出）；**错误回合后 agy 自行 exit 1**（agy 上游 CLI 行为，非我方误杀）。已排除 antigravityProcessScan（只杀 bg-updater）、supervisor 闲置回收（不存在）、print-timeout（生产 24h）。兜底链本就健全：turn 外进程死亡走 onClose→inactive、下条消息 restartThread 恢复；turn 内 startTurn 有 transparent respawn。
- **Gemini 跑完错误 toast 根治**：真凶是「双重失败」——错误回合时 agy 先吐 `result status=ERROR`（canonicalizer 发 `error` + `turn.completed(failed)`，第一条 toast 带真实错误文案，合法），**0.4s 后 agy 自行 exit 1**（agy-exit-probe 实证的上游行为）→ transport 发 NATIVE_PROCESS_CRASHED 诊断 → `handleDiagnostic` 在无活跃 turn 时走 `listener.onError` → `failStructuredSession` 再发第二条 toast（「Native process exited with code 1 (none).」——无用文案盖掉真错误）+ 合成 error 事件 + 随机 turnId 的合成 turn.completed。修复（`src/supervisor/agents/antigravity/structuredSession.ts`）：①无活跃 turn 的 NATIVE_PROCESS_CRASHED 降级为 warning 事件（生命周期由 handleProcessExit→onClose→inactive→下条消息 transparent respawn 覆盖，不再是第二次线程失败）；②`statusForEvent` 不再因孤立 `error` 事件翻线程状态——所有真失败路径都紧跟 `turn.completed(failed)`，中途/回合后噪音帧不得翻状态；③无活跃 turn 时 canonical error 事件降级 warning。另统一 canonicalizer `turnState` 的 kind 判定（`nativeEventCanonicalizer.ts`）：DSH 的 snake_case kind（`max_tokens`）此前不做下划线归一会静默落成 `completed`，与 adapter 侧 `deepSeekReasonKind` 的归一（`replace(/_/g,"-")`）不一致——现两处同规则。注：agy 从不发 `turn/end`，该路径只影响 DeepSeek。验证：`structuredSession.test.ts` 加 3 例（错误回合后 exit 1 降级 warning 且 onError 不触发/onClose 仍恢复、成功回合后闲置崩溃降级、回合中崩溃仍正常拒绝），`nativeEventCanonicalizer.test.ts` 加 3 例（`max_tokens`→failed、连字符形保持、未知 kind 保持 completed）；antigravity + nativeHarness + threadSession 套件 419 过 / 5 失败已 stash 对照证实为 HEAD 既有基线；typecheck PASS；触碰文件 oxlint 0 警告。
- 顺手修复既有 WIP 类型错误：`queuedFollowUpActions.test.ts:78` `vi.mock` 工厂类型不传导导致 `mockRejectedValueOnce` 报 TS2339，改 `vi.mocked(...)`。本批验证：`pnpm typecheck` PASS；触碰文件 oxlint 0 警告。
- **标题栏版本号点击 = 手动检查更新**：`MainTitlebar.tsx` 版本号从静态 span 改为按钮（保留 `data-testid="titlebar-app-version"`），点击调 `checkForUpdate({})`（手动语义、失败 toast）；checking 中防重入；订阅 store 观察 checking→非 checking 跳变，落 `idle` 时 toast「CraftStation 已是最新版本」（更新可用/错误走既有状态通道，不重复通知）。ControlTooltip 提示「点击检查 CraftStation 新版本」。验证：`MainTitlebar.test.tsx` 加 1 例，10/10 全过。
- **Issue #10 Devin 登录闪退修复**：`devin` 不在 PATH 时，PowerShell 直接 `CommandNotFoundException` 把登录终端秒关，用户来不及看到任何提示。修复：①`runAgentLoginCommand`（`agentLoginActions.ts`）新增 `resolveLoginBinaryCommand`——从 agent status（WSL 项目用 wsl 池）取该 provider 已安装的绝对 `executablePath`，命中则把登录模板首 token 改写为绝对路径调用（Windows `& '<path>'` / posix `'<path>'`），未命中回退裸名；对 `CLI_LOGIN_COMMANDS` 全部 6 个 provider 生效。②`devin/detection.ts` 的 `loginCommand` 从静态裸名改为 kimi 式函数形态（无 executablePath 时返回 undefined）。验证：`agentLoginActions.test.ts` 加 3 例（Windows 绝对路径改写/无路径回退/WSL 池）、`devin.test.ts` loginCommand 断言更新，48/48 全过。
- **Issue #12 任务栏图标任务完成指示**：新模块 `src/main/taskbarAttention.ts`——镜像渲染层 `notifications.ts` 的 classifyTransition 语义（error→error、needs_*→needsAttention、active→idle/finished→done、用户主动 stop 不算完成），仅当主窗口**无焦点**时给该 threadId 打标 → `setOverlayIcon`（内嵌 32×32 琥珀圆点 PNG，打包无外部资源依赖）+ `flashFrame(true)`（macOS 加 dock bounce）；主窗口 focus 停闪但徽标保留，用户打开对应线程（`openThread` → 新 `dismissTaskbarAttention` IPC）才撤标；`thread-exited` 自动撤标；走设置总闸 `notificationsEnabled` + `notificationStatuses[category]` 实时门控（与 `isCloseToTrayEnabled` 同读法，改设置免重启）。接线：main.ts onEvent tap + 窗口 focus 监听 + IPC handler + shutdown dispose；`IPC_WINDOW_CHANNELS.taskbarAttentionDismiss`；preload invoke；mobile bridge 注册 no-op（无桌面任务栏）。验证：`taskbarAttention.test.ts` 12 例全过（跳变打标/有焦点不打/分类门控/forceClose 忽略/dismiss 清标/焦点停闪留标/thread-exited 自动撤/dispose 清理/macOS dock bounce/无窗口容错）。
- **Issue #11 更新下载进度一直 0% 改善**：真因是 GitHub assets CDN 慢时前几十 MB 百分比长时间显示 0%（120s 无字节停滞 guard 在 1.2.15 已有），用户无法分辨「卡住」还是「在下」。修复（纯 UX，`MainTitlebar.tsx`）：①标题栏下载 pill 与更新菜单下载项在百分比后追加字节进度+速度（`2.2 MB / 85.0 MB · 110.0 KB/s`，复用 `formatBytes` + store 既有 `downloadTransferred/downloadTotal/downloadBytesPerSecond` 字段，与侧边栏/About 同数据）；②下载中菜单新增「改为在浏览器中下载」项，点击 `openExternal` 打开 GitHub Releases 页兜底。验证：`CliUpdateMenu.test.tsx` 加 1 例（字节/速度可见 + 浏览器项点击断言 URL），8/8 全过。
- 本轮验证汇总：`taskbarAttention` 12/12、`MainTitlebar` 10/10、`CliUpdateMenu` 8/8、`agentLoginActions`+`devin` 48/48、`threadActions` 49/49 全过；`pnpm typecheck` PASS；触碰文件 oxlint 0 警告；i18n 已 extract，zh-CN 新文案补齐（「CraftStation 已是最新版本」「点击检查 CraftStation 新版本」「改为在浏览器中下载」等）。`SortableThreadItem.test.tsx` 2 例失败为 HEAD 既有基线（「unload」菜单项在当前源码中不存在，该测试 mock 了 threadActions，与本批无关）。
...
- **Devin 内置 MCP 真正注入**：新版 Devin CLI 在 ACP initialize 广告 `mcpCapabilities:{http:false,sse:false}`，但整体 `advertised ?? assumed` 解析把假定支持 HTTP 的能力静默丢弃——探针实测 devin 3000.10.27 的 `session/new` 接受 HTTP MCP（广告是假的）。`resolveAcpMcpCapabilities` 改为按传输合并且 assumed `true` 盖过低报广告；真被拒时开会话自动摘除这部分服务器重试。
- **MCP 注入盲区上报**：`AcpStructuredSession.consumeMcpInjectionDrops` 记录 open 未送达的服务器名，spawn 管线在启动/重启/换 Provider 三处经新 `thread-mcp-injection-drop` 事件上报，渲染层 toast + 铃铛通知栏积累（只含服务器名）。
- **K3-256k 上下文容量修正**：模型 id `-NNNk` 后缀（产品契约）优先于 Kimi ACP probe 对 K3 家族统一报的 1M，k3-256k 正确显示 256K。
- **Kimi 压缩文案过滤**：`stripKimiHarnessNoise` 过滤 `Compacting conversation context…` 等 6 种状态行（text/thought/native visibleText 三路径）。
- **错误 toast 修复**：HeroUI `createToastFunction` 重建 content 时静默丢弃 `context`/`onPress`/自定义字段（既有隐性 bug：线程完成 toast 的状态行与点击跳转从未生效），新增 `toastExtras.ts` 原地包装共享 toast 变体方法转发 extras；danger 变体错误 toast 标题不再截断（pre-wrap 全文显示）且自动积累进铃铛通知面板（`ledgerLogged` 标记防重复入栏）。
- **子代理死后状态不收敛修复（「已工作 7h57m」永不消失）**：DB 中 `isSubAgent` item 永久卡在 running。根因：`createKimiBackgroundBridge` 的 `resolveKimiSessionDir` 用 `nativeKimiHomePath()`（只认主进程 env / `~/.kimi-code`），而托管账号线程的 `KIMI_CODE_HOME` 在 `baseSpawnEnv` 里指向 per-profile 目录——桥在错误的根下等会话目录，且 `sessionDirTimeoutMs` 默认 0 = 无限静默等待，任务完成事件永远发不出。修复：bridge 接受线程实际 `kimiHome`；会话目录等待默认 10 分钟上限，超时以 lost 收敛并打诊断；`dispose()` 对 pending launch 以 lost 补发终态（先于 session dispose，emission 能落地）；`onSettled` 同步 settle 消除 dispose 竞态。
- **Muse Spark 思考链不并行渲染修复**：max 不是 muse-spark 合法 reasoning variant（合法仅 minimal/low/medium/high/xhigh），派发方不校验是诱因之一；真正的渲染 bug 在 `chatPaneSelectors.isToolGroupItem` 把带非空 `streams.reasoning_text` 的 reasoning 条目当成工具组可折叠胶水。现在带文本的思考折出工具组原位渲染，思考与工具调度交错并行展示。
- **Schedule MCP by-id 写路径修复**：OpenCode bridge 把 null 序列化成字符串 `"null"`，`update timezone:null` 被判非法时区——`nullishArg` preprocess 把 `"null"`/`"undefined"`/`""` 归一为 null 应用于五个 nullable schema；`ScheduleService.start()` 增加全量一致性自检（list 可见 id 必须 get 可读，失败打 `STORE_INDEX_MISSING` ERROR 诊断）；`sweepUnavailableBindings` 绑定检查抛异常时保留记录不再误删；`requireSchedule`/`requireTask` 报错区分「list 可见但 by-id 不可读」。
- **排队消息图片保留 + 编辑回到聊天框**：排队条此前只渲染纯文本（图片视觉消失），且行内编辑 `updateQueuedFollowUpPrompt` 把 segments 整体替换为单条 text（附件真丢失）。修复：`ThreadQueuedFollowUpStrip` 重写——从 `queued.segments` 提取 attachment 段渲染图片缩略图/文件名 chip，去掉行内编辑；桌面端 Edit 走 `handleEditQueuedFollowUp`（文本段回 editor、附件段经 `attachmentFromSegment` 重建并与现有附件合并 restore、消费队列条目、聚焦）；移动端 strip 是 composer sibling，Edit 改为把完整 segments 投递 `useComposerInputInbox`（key=thread.id），composer drain effect 拆出 attachment 段入附件栏、文本段插入 editor；`updateQueuedFollowUpPrompt` 已从 slice 删除。
- **Bug D（muse 线程显示"说是 gemini"）归因**：数据层全链路正常——线程 config/上游 opencode 会话/743 条消息/usage_events/bindings 均为 `opencode-go/muse-spark-1.3-contributor`，会话文本无 gemini 自称。已向用户请求指出 "Gemini" 字样的具体界面位置（疑似 effort 下拉出现 Gemini 档位，与 max 非法派发同源），待用户补图后再修。
- 验证：`turnRetryCoordinator.test.ts` 10 例全过；`session.test.ts` 新增 3 例（assumed 盖过低报广告并送达、兼容报错回退并上报、无能力丢弃并上报），acp/factory/userMcp/devin 481 例全过；toast 36 例、Kimi detection+noise 全过；kimi agents 套件 143 例全过；chatPaneSelectors/ToolCallGroup 69 例全过；schedules 66 例全过；`ThreadComposerSection.test.tsx` 队列套件 7 例全过（strip 缩略图、Edit 回带附件、inbox attachment drain；文件总 55 过 / 1 失败为干净 HEAD 基线，已 stash 证实）；typecheck PASS；全部触碰文件 oxlint 0 警告；i18n 已 extract（zh-CN 齐全）；`src/supervisor/runtime/` 947 过 / 6 失败（干净 HEAD 基线，已 stash 对比证实）；`runtime.test.ts` 失败名单与干净 HEAD 逐行一致。

## Release 1.2.16 — 启动同步检查更新 + Gemini 闪终端窗根治（2026-09-17，已发布为 Latest）

- 已提交 `73ac397` 并 push，tag `v1.2.16` 已推送；双包 + blockmap + `latest.yml` 共 4 文件已上传到 GitHub Release v1.2.16（Latest）。便携版备份保留 1.2.16 + 1.2.15；中间产物（`win-unpacked/`、`builder-debug.yml`）已清。
...
- 用户已验收，按惯例提交推送并双包发布（release notes 见 `ai_workspace/release-notes-1.2.16.md`）。
