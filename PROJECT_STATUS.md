...
- 当前执行 Feature：`v1.0.1 — Native CLI Multi-Account Profile Runtime`。worktree `.worktrees/v1.0.1-native-profile-runtime`，分支 `dev/v1.0.1-native-profile-runtime`，基线 `main@f5a4bb2`。Manager Plan：该 worktree 的 `ai_workspace/agent_docs/manager_1.0.1.md`。Status：`FIX #2 DELIVERED / AWAITING DEBUGGER RE-REVIEW #2`（`66ba9eb`）。初审查 `FAIL / FIX #2`；Fix #2 由 Debugger 代改后由 Coder 验证并提交（secret boundary 拆分、Codex `account/read` fail-closed 门禁、Kimi 控制面）；Codex/Kimi 真实 native receipt 仍 `UNVERIFIED`，不得表述为 PASS，`report_1.0.x` 未生成。
- 并行 Feature：`v1.1.0 — Compatibility Bridge & Model × Harness Composition`。worktree `.worktrees/v1.1.0-compatibility-bridge`，分支 `dev/v1.1.0-compatibility-bridge`，基线 `main@f5a4bb2`。Status：`DEBUGGER FAIL / BLOCKED`（`debugger_1.1.2.md`、`report_1.1.md`）；实现+两轮 Fix+takeover 加固已全部固化为候选 `fde6c87`。真实 CPA 运行时证据、独立 Compatibility Adapter、官方 Target Harness 消费、账号 namespace 绑定与真实 Agent Loop 未证明；Debugger 未授权直接再修，需 Manager re-plan。
- 并行 Feature：`v1.2.0 — Unified MCP + Skills Capability Foundation`。worktree `.worktrees/v1.2.0-mcp-skills-capability`，分支 `dev/v1.2.0-mcp-skills-capability`，基线 `main@f5a4bb2`。Status：`FIX CYCLE v1.2.0.1 DELIVERED / AWAITING DEBUGGER RE-REVIEW`（`de052d3`）。初审查 `FAIL`（`debugger_1.2.0.1.md`）；F1–F8 已全部修复（无 ID Auto/Efficient 候选快照、`$skill` 前缀、WSL 感知 resolver、`imported` origin、Managed Skill 强制 copy、changelog 唯一性、capabilityMode 端到端接线、NUL 分隔符），typecheck/lint/build/聚焦套件全绿，全量 vitest 仅余 2 个 jsdom 负载 flaky（单跑通过，已分类）。产品版本号定为 `1.7.0`（历史 `1.2.0` 已被 2026-06-05 发布占用）。
