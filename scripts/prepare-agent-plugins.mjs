 ...
 * (`forward.mjs`, or OpenCode's in-process `craftstation-status.mjs`). This keeps
 * packaging registration beside the provider instead of duplicating a list in
 * this script.
 *
 * Plus a shared forwarder runtime under `_runtime/craftstation-hook-runtime.mjs`
...
const PROVIDER_RUNTIME_ASSETS = ["forward.mjs", "craftstation-status.mjs"];
...
export function resolveSharedForwardRuntime(sourceAgentsDir) {
  const runtime = {
    src: join(sourceAgentsDir, "plugin", "forward-runtime", "craftstation-hook-runtime.mjs"),
    destRel: join("_runtime", "craftstation-hook-runtime.mjs"),
  };
