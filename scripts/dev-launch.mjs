// Electron-based hosts (VS Code, CraftStation production build) set
...
const env = {
  ...process.env,
  VITE_DEV_SERVER_URL:
    process.env.CRAFTSTATION_DEV_APP_URL ?? `http://127.0.0.1:${resolveDevServerPort()}`,
};
const cdpUserDataDir = process.env.CRAFTSTATION_CDP_USER_DATA_DIR?.trim();
if (cdpUserDataDir) {
