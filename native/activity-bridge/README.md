...
Local Capacitor 8 plugin bridging the CraftStation mobile web layer to iOS
...
```jsonc
// package.json
"@craftstation/activity-bridge": "file:native/activity-bridge"
```
...
- **CocoaPods** (default for `cap add ios`) — `CraftStationActivityBridge.podspec`.
...
in both the app plugin target and the `CraftStationActivities` widget-extension
