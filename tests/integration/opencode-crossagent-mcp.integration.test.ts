...
describe("opencode hosts Crossagents MCP on its shared server (live)", () => {
  ...
  beforeAll(async () => {
    firstProjectDir = mkdtempSync(join(tmpdir(), "craftstation-oc-shared-a-"));
    secondProjectDir = mkdtempSync(join(tmpdir(), "craftstation-oc-shared-b-"));
    ...
    ingress = new OwnSubagentsMcpIngress({
      ...
      resolveProviderSessionThreadId: (sessionId: string) => providerSessions.get(sessionId),
    });
    await ingress.start();
  ...
  it("reuses one server and routes the shared credential by session id", async () => {
    ...
    try {
      ...
      const created = await first.client.session.create({
        directory,
        title: "craftstation/crossagents-shared-test",
      });
