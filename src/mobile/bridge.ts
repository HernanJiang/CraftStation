...
/**
 * Remote-session implementation of `window.craftstation`. The desktop renderer
...
export function setRemoteBridgeClient(
  ...
  // craftstation-local <img> sources load only inside the desktop's Electron
...
/**
 * Maps a craftstation-local image URL to the desktop's authenticated image
...
export function installRemoteBridge(): void {
  ...
  window.craftstation = remoteBridge as unknown as CraftStationBridge;
}
