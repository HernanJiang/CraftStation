...
function makeBaseDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "craftstation-gemini-plugin-"));
  tempDirs.push(dir);
  return dir;
}
...
beforeEach(() => {
  ...
  savedBrowserMcpEnv = {
    ...(process.env.CRAFTSTATION_BROWSER_MCP_URL !== undefined
      ? { url: process.env.CRAFTSTATION_BROWSER_MCP_URL }
      : {}),
    ...(process.env.CRAFTSTATION_BROWSER_MCP_TOKEN !== undefined
      ? { token: process.env.CRAFTSTATION_BROWSER_MCP_TOKEN }
      : {}),
  };
...
afterEach(() => {
  ...
  if (savedBrowserMcpEnv.url !== undefined) {
    process.env.CRAFTSTATION_BROWSER_MCP_URL = savedBrowserMcpEnv.url;
  }
  if (savedBrowserMcpEnv.token !== undefined) {
    process.env.CRAFTSTATION_BROWSER_MCP_TOKEN = savedBrowserMcpEnv.token;
  }
});

describe("getGeminiPluginPaths", () => {
  it("places Gemini settings under CraftStation's plugin dir", () => {
...
describe("renderGeminiSettings", () => {
  it("renders only the trimmed hook surface with the resolved-node command prefix", () => {
    const commandPrefix =
      "'/home/demo/.nvm/versions/node/v22.11.0/bin/node' '/home/demo/.craftstation/agent-plugins/gemini/forward.mjs'";
    ...
    expect(doc.hooks.AfterAgent?.[0]?.hooks[0]).toMatchObject({
      name: "craftstation-status-AfterAgent",
      type: "command",
...
describe("installGeminiPlugin", () => {
  it("stages assets and writes a private Gemini system settings file", () => {
    ...
    expect(command).toMatch(/agent-plugins[\\/]+gemini[\\/]+craftstation-hook\.(?:sh|cmd|ps1)/);
