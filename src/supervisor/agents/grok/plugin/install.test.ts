...
describe("grokIntentFor", () => {
  it("maps Grok's registered lifecycle events to CraftStation intents", () => {
...
describe("installGrokPlugin (native, global hook write)", () => {
  function makeNativeCtx() {
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-grok-stage-"));
    const grokDir = mkdtempSync(join(tmpdir(), "craftstation-grok-home-"));
    const envKind = process.platform === "win32" ? ("windows" as const) : ("posix" as const);
    return { baseDir, grokDir, ctx: { envKind, baseDir } };
  }

  it("writes ~/.grok/hooks/craftstation-status.json at install time", () => {
  ...
  it("removes the legacy CraftStation global hook after install", () => {
    const { grokDir, ctx } = makeNativeCtx();
    const legacyPath = join(grokDir, GLOBAL_HOOK_DIR_NAME, "craftstation-status.json");
    mkdirSync(join(grokDir, GLOBAL_HOOK_DIR_NAME), { recursive: true });
    writeFileSync(
      legacyPath,
      JSON.stringify(
        renderGrokHookConfig({
          command: "'/home/u/.craftstation/agent-plugins/grok/craftstation-hook.sh'",
        }),
      ),
    );
  ...
  it("does not touch any project-level paths", () => {
    const { grokDir, ctx } = makeNativeCtx();
    const projectDir = mkdtempSync(join(tmpdir(), "craftstation-grok-proj-"));
...
describe("getGrokPluginPaths", () => {
  it("returns staging dir under provided baseDir for native ctx", () => {
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-grok-paths-"));
...
describe("isGrokPluginInstalled", () => {
  function stage(parts: {
    ...
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-grok-verify-"));
