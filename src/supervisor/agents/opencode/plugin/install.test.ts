...
function makeBaseDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "craftstation-opencode-plugin-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("getOpenCodePluginPaths", () => {
  it("places craftstation-managed staging under the supplied base dir", () => {
    ...
    expect(paths.opencodePluginFile).toBe(
      join(process.env.OPENCODE_CONFIG_DIR, "plugins", "craftstation-status.js"),
    );
  });
  ...
  it("uses ~/.config/opencode/plugins when OPENCODE_CONFIG_DIR is unset", () => {
    ...
    delete process.env.OPENCODE_CONFIG_DIR;
    const paths = getOpenCodePluginPaths({ envKind: "posix", baseDir });
    expect(paths.opencodePluginFile.endsWith(join("plugins", "craftstation-status.js"))).toBe(true);
  });
});
...
describe("installOpenCodePlugin", () => {
  it("stages assets and drops the plugin + manifest into OpenCode's plugins dir", () => {
    ...
    // CraftStation-side staging
    expect(existsSync(join(result.paths.pluginDir, "plugin.json"))).toBe(true);
    expect(existsSync(join(result.paths.pluginDir, "craftstation-status.mjs"))).toBe(true);

    // OpenCode-side drops: .js plugin file (auto-discovered) + sibling manifest
    const droppedPlugin = join(opencodeDir, "plugins", "craftstation-status.js");
    const droppedManifest = join(opencodeDir, "plugins", "craftstation-status.plugin.json");
    ...
    expect(
      readFileSync(join(result.paths.pluginDir, "craftstation-status.mjs")).equals(
  ...
  it("overwrites private Crossagents routing metadata from the trusted tool context", async () => {
    ...
    try {
      const output = { args: { __craftstation_provider_session_id: "forged", prompt: "review" } };
      await hooks["tool.execute.before"]?.(
        { tool: "crossagents_run_agent", sessionID: "session-trusted" },
        output,
      );
      expect(output.args.__craftstation_provider_session_id).toBe("session-trusted");
  ...
  it("scrubs a stale `file://` plugin entry that older builds left in opencode.json", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        plugin: [
          ...
          `file:///prior/baseDir/agent-plugins/opencode/craftstation-status.mjs`,
        ],
      }),
      "utf8",
    );
    ...
    // CraftStation entry removed; user's other plugin entry is preserved.
  ...
  it("removes the `plugin` key entirely when ours was the only entry", () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: [`file:///prior/baseDir/agent-plugins/opencode/craftstation-status.mjs`],
      }),
      "utf8",
    );
  ...
  it("preserves user MCP entries whose names match CraftStation built-ins", () => {
    const baseDir = makeBaseDir();
    const opencodeDir = makeBaseDir();
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;
    const configPath = join(opencodeDir, "opencode.json");
    const userMcp = Object.fromEntries(
      ["browser", "crossagents", "chrome", "computer_use", "craftstation"].map((name) => [
        name,
  ...
  it("restores entries shadowed by the retired MCP config projection", () => {
    ...
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;
    const configPath = join(opencodeDir, "opencode.json");
    const sidecarPath = join(opencodeDir, ".craftstation-managed-mcp.json");
    const originalBrowser = { type: "local", command: ["user-browser"] };
    writeFileSync(
      configPath,
      JSON.stringify({
        mcp: {
          browser: { type: "remote", url: "http://craftstation/browser" },
          craftstation_only: { type: "remote", url: "http://craftstation/only" },
        },
      }),
      "utf8",
    );
    writeFileSync(
      sidecarPath,
      JSON.stringify({ browser: originalBrowser, craftstation_only: null }),
      "utf8",
    );
  ...
  it("removes a legacy craftstation-status.mjs left behind by an older install", () => {
    const baseDir = makeBaseDir();
    const opencodeDir = makeBaseDir();
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;

    const legacyPath = join(opencodeDir, "plugins", "craftstation-status.mjs");
  ...
  it("removes a legacy craftstation-status.js left behind by the old brand", () => {
    const baseDir = makeBaseDir();
    const opencodeDir = makeBaseDir();
    process.env.OPENCODE_CONFIG_DIR = opencodeDir;

    const legacyPath = join(opencodeDir, "plugins", "craftstation-status.js");
    mkdirSync(dirname(legacyPath), { recursive: true });
    writeFileSync(legacyPath, "// stale legacy plugin\n");
  ...
  it("treats a missing dropped manifest as not-installed", () => {
    ...
    unlinkSync(join(opencodeDir, "plugins", "craftstation-status.plugin.json"));
  ...
  it("removes dropped files, staging dir, and the opencode.json entry", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        plugin: [
          "@user/other-plugin",
          `file://${result.paths.pluginDir.replace(/\\/g, "/")}/craftstation-status.mjs`,
        ],
    ...
    expect(existsSync(join(opencodeDir, "plugins", "craftstation-status.plugin.json"))).toBe(false);
