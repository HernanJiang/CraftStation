...
describe("installCopilotPlugin (native, global hook write)", () => {
  function makeNativeCtx() {
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-copilot-stage-"));
    const copilotDir = mkdtempSync(join(tmpdir(), "craftstation-copilot-home-"));
    const envKind = process.platform === "win32" ? ("windows" as const) : ("posix" as const);
    return { baseDir, copilotDir, ctx: { envKind, baseDir } };
  }

  it("writes ${COPILOT_HOME}/hooks/craftstation-status.json at install time", () => {
    ...
    expect(
      process.platform === "win32"
        ? /craftstation-hook\.ps1' sessionStart$/.test(powershellCommand ?? "")
        : powershellCommand,
  ...
  it("removes the legacy CraftStation global hook after install", () => {
    const { copilotDir, ctx } = makeNativeCtx();
    const legacyPath = join(copilotDir, GLOBAL_HOOK_DIR_NAME, "craftstation-status.json");
  ...
  it("does not touch any project-level paths", () => {
    const { copilotDir, ctx } = makeNativeCtx();
    const projectDir = mkdtempSync(join(tmpdir(), "craftstation-copilot-proj-"));
...
describe("getCopilotPluginPaths", () => {
  it("returns staging dir under provided baseDir for native ctx", () => {
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-copilot-paths-"));
...
describe("isCopilotPluginInstalled", () => {
  function stage(parts: {
    ...
    const baseDir = mkdtempSync(join(tmpdir(), "craftstation-copilot-verify-"));
