...
export function createCopilotAdapter(): AgentAdapter {
  ...
  return {
    ...
    pluginId: "craftstation-status@copilot",
    ...
    // `${COPILOT_HOME ?? ~/.copilot}/hooks/craftstation-status.json` written at
    // install time, and `CRAFTSTATION_HOOK_*` env is injected by the coordinator.
