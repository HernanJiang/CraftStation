...
describe("custom MCP translators", () => {
  ...
  it("keeps OpenCode launch credentials out of the inline config", () => {
    ...
    expect(config.mcp["local.tools"]?.environment?.MODE).toMatch(/^\{env:CRAFTSTATION_MCP_/u);
    expect(config.mcp.remote?.headers?.Authorization).toMatch(/^\{env:CRAFTSTATION_MCP_/u);
