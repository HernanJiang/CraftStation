...
function makeTempDir(prefix = "craftstation-installer-base-"): string {
...
describe("createPluginSourceResolver", () => {
  const ENV_KEY = "CRAFTSTATION_TEST_PLUGIN_SOURCE";
...
describe("isPluginAssetsFresh + copyPluginAssetsIfStale", () => {
  function seedSource(): string {
    const sourceDir = makeTempDir("craftstation-installer-base-src-");
...
describe("renderNativeHookWrapper", () => {
  ...
  it("prefers PowerShell 7, then Windows PowerShell, before cmd fallback on Windows", () => {
    const body = renderNativeHookWrapper({ electronPath: "C:\\CraftStation\\CraftStation.exe" });
    ...
    expect(body).toContain(
      'pwsh.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0craftstation-hook.ps1" %*',
    );
    expect(body).toContain(
      'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0craftstation-hook.ps1" %*',
    );
  });
});
...
describe("renderNativeHookPowerShellWrapper", () => {
  it("passes hook args through to forward.mjs", () => {
    const body = renderNativeHookPowerShellWrapper({
      electronPath: "C:\\CraftStation\\CraftStation.exe",
    });
...
describe("buildNativeHookCommandHead", () => {
  ...
  it("prefers PowerShell 7 for native Windows hook wrappers", () => {
    const commandHead = buildNativeHookCommandHead("C:\\Users\\u\\craftstation-hook.cmd", (name) =>
      name === "pwsh.exe" ? "C:\\Program Files\\PowerShell\\7\\pwsh.exe" : undefined,
    );
    const expected =
      process.platform === "win32"
        ? '"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\u\\craftstation-hook.ps1"'
        : "'C:\\Users\\u\\craftstation-hook.cmd'";
  ...
  it("falls back to Windows PowerShell when PowerShell 7 is missing", () => {
    ...
    const expected =
      process.platform === "win32"
        ? '"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\u\\craftstation-hook.ps1"'
        : "'C:\\Users\\u\\craftstation-hook.cmd'";
    expect(commandHead).toBe(expected);
  });
  ...
  it("preserves an extensionless resolved PowerShell command", () => {
    ...
    const expected =
      process.platform === "win32"
        ? '"C:\\Tools\\pwsh" -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\u\\craftstation-hook.ps1"'
        : "'C:\\Users\\u\\craftstation-hook.cmd'";
    expect(commandHead).toBe(expected);
  });
  ...
  it("escapes embedded quotes for the active native shell", () => {
    ...
    const expected =
      process.platform === "win32"
        ? '"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\a\\"b\\craftstation-hook.ps1"'
...
describe("buildNativeHookCmdShellCommand", () => {
  ...
  it.skipIf(!isWindows)(
    ...
    () => {
      const command = buildNativeHookCmdShellCommand("C:\\Users\\u\\craftstation-hook.cmd");
      expect(command).toBe('cmd.exe /d /s /c call "C:\\Users\\u\\craftstation-hook.cmd"');
      expect(command).not.toMatch(/pwsh|powershell/i);
    },
  );

  it.skipIf(isWindows)("returns a single-quoted wrapper path on POSIX", () => {
    const command = buildNativeHookCmdShellCommand("/home/u/craftstation-hook.sh");
    expect(command).toBe("'/home/u/craftstation-hook.sh'");
  });
});

describe("buildNativeHookCommandHeads", () => {
  it("centralizes generic, bash, and PowerShell native command shapes", () => {
    const heads = buildNativeHookCommandHeads("C:\\Users\\u\\craftstation-hook.cmd", (name) =>
      name === "pwsh.exe" ? "C:\\Program Files\\PowerShell\\7\\pwsh.exe" : undefined,
    );
    ...
    const expected =
      process.platform === "win32"
        ? {
            command:
              '"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\u\\craftstation-hook.ps1"',
            bashCommand: "'C:\\Users\\u\\craftstation-hook.cmd'",
            powershellCommand: "& 'C:\\Users\\u\\craftstation-hook.ps1'",
          }
        : {
            command: "'C:\\Users\\u\\craftstation-hook.cmd'",
            bashCommand: "'C:\\Users\\u\\craftstation-hook.cmd'",
          };
