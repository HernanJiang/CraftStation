 ...
 * the responsible parent (CraftStation). The result is a launch-time "would like
 * to access data from other apps" prompt unrelated to anything the user asked
 * CraftStation to do. Pointing every probe at an empty, CraftStation-owned
...
export function getAgentProbeCwd(location: ProjectLocation): string {
  ...
  const probeDir = cachedProbeDir ?? join(resolveCraftStationBaseDir(), "agent-probe");
  try {
