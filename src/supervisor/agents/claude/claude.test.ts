...
describe("createClaudeAdapter handleOscTitle", () => {
  ...
  // Observed from real dev sessions (~/.craftstation/logs/terminal/*.log):
...
describe("createClaudeAdapter handleOscNotification (iTerm2 OSC 9;4 progress)", () => {
  ...
  // Real bodies observed in ~/.craftstation-dev/logs/terminal/*.log after the
...
describe("createClaudeProfileAdapter", () => {
  ...
  it("creates a distinct Claude adapter backed by a separate config directory", () => {
    ...
    const expectedConfigDir = path.join(homedir(), ".craftstation/claude-profiles/work");
    expect(
  ...
  it("merges the instance environment into the spawn env, with CLAUDE_CONFIG_DIR winning", () => {
    ...
    const expectedConfigDir = path.join(homedir(), ".craftstation/claude-profiles/glm");
  ...
  it("restricts the effort allow-list and keeps an allowed default", () => {
    const adapter = createClaudeProfileAdapter({
      ...
      driver: "claude",
      displayName: "GLM",
      config: { configDir: "~/.craftstation/claude-profiles/glm", efforts: ["high", "max"] },
    });
  ...
  it("applies an external provider's default and per-model effort choices", () => {
    const adapter = createClaudeProfileAdapter({
      ...
      displayName: "Kimi",
      config: {
        configDir: "~/.craftstation/claude-profiles/kimi",
  ...
  it("re-homes the default effort to the first allowed tier when disabled", () => {
    const adapter = createClaudeProfileAdapter({
      ...
      driver: "claude",
      displayName: "GLM",
      config: { configDir: "~/.craftstation/claude-profiles/glm", efforts: ["max", "ultracode"] },
    });
  ...
  it("falls back to the built-in efforts when an allow-list has no known tiers", () => {
    ...
    const adapter = createClaudeProfileAdapter({
      ...
      driver: "claude",
      displayName: "GLM",
      config: { configDir: "~/.craftstation/claude-profiles/glm", efforts: ["bogus", "nope"] },
    });
