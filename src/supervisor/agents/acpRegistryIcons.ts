 ...
 * `craftstation-local://` URL pointing at the cached file. The renderer paints the
 * icon via CSS `mask-image` in `ProviderIcon`; the `craftstation-local` scheme must
...
/**
 * Cache a remote ACP registry icon to disk and return a `craftstation-local://`
