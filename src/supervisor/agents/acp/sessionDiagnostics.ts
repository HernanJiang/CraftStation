 ...
 *
 * Off by default. Set `CRAFTSTATION_ACP_LOG=toolcalls` to capture just
 * `tool_call` / `tool_call_update` updates (what we use to design
 * per-adapter wire-format transforms); set `CRAFTSTATION_ACP_LOG=full` to
...
const ACP_LOG_MODE = (() => {
  const mode = process.env.CRAFTSTATION_ACP_LOG;
...
export function maybeCaptureAcpUpdate(
  ...
  try {
    const dir = resolveCraftStationPaths(process.env.CRAFTSTATION_DATA_DIR).logsDir;
    if (!acpLogDirEnsured) {
