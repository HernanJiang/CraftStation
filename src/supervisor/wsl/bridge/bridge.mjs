#!/usr/bin/env node
/**
 * CraftStation in-distro bridge server — runs INSIDE a WSL distro.
 *
...
const PROTOCOL_VERSION = Number(process.env.CRAFTSTATION_HOOK_PROTOCOL_VERSION ?? "1") || 1;
...
if (!SECRET) {
  emit({ type: "error", message: "CRAFTSTATION_HOOK_SECRET missing in bridge env" });
  process.exit(2);
}
...
function resolveBrowserMcpUpstreamCandidates() {
  const raw = process.env.CRAFTSTATION_BROWSER_MCP_URL;
  if (!raw) return [];
  let parsed;
  try {
    parsed = new URL(raw);
...
function sanitizeGitEnv(baseEnv) {
  ...
  delete env.CRAFTSTATION_HOOK_SECRET;
  delete env.CRAFTSTATION_HOOK_PROTOCOL_VERSION;
  delete env.GIT_DIR;
...
function gitCheckpointSnapshotHandler(req, body) {
  ...
  if (typeof body.ref !== "string" || !CRAFTSTATION_CHECKPOINT_REF_RE.test(body.ref)) {
  ...
  try {
    ...
    const tempIndex = `${indexPath}.craftstation-${randomUUID()}`;
    try {
      ...
      const message = `CraftStation checkpoint\n\n${JSON.stringify(body.metadata)}\n`;
...
async function handleMcpProxy(req, res) {
  ...
  const upstreamToken = process.env.CRAFTSTATION_BROWSER_MCP_TOKEN;
