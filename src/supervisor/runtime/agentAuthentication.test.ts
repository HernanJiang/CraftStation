...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "craftstation-runtime-auth-"));
  tempDirs.push(dir);
  return dir;
}
...
function writeGenericAcpSettings(
  ...
  const { settingsPath } = resolveCraftStationPaths(dataDir);
  writeFileSync(
    settingsPath,
    JSON.stringify(
      {
...
describe("authenticateAcpAgent", () => {
  ...
  it("clears generic ACP auth when browser login does not complete", async () => {
    ...
    await expect(
      ...
      "My ACP reported authentication success, but CraftStation could not verify it. Configure My ACP directly, then try again.",
    );
