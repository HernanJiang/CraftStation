...
import type { HostPort, OAuthToken, UsageSnapshot } from "@craftstation/agents-usage";
...
function tempCachePath(): string {
  const path = join(tmpdir(), `craftstation-usage-test-${process.pid}-${cachePaths.length}.json`);
...
describe("UsageService", () => {
  ...
  it("collects Claude profile usage from the profile config directory", async () => {
    const profileDir = join(tmpdir(), `craftstation-usage-claude-profile-${process.pid}`);
