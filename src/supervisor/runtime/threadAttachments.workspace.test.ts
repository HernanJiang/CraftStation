...
describe("rewriteSegmentsForWorkspace", () => {
  it("copies an out-of-workspace attachment into .craftstation/attachments and rewrites the path", async () => {
    ...
    const dest = join(project, ".craftstation", "attachments", "shot.png");
    ...
    expect(readFileSync(join(project, ".craftstation", ".gitignore"), "utf8")).toContain("*");
  });
  ...
  it("leaves attachments already inside the workspace untouched", async () => {
    ...
    expect(existsSync(join(project, ".craftstation"))).toBe(false);
  });
