...
initializeSupervisorSentry({
  appVersion: process.env.CRAFTSTATION_APP_VERSION ?? process.env.npm_package_version ?? "dev",
  isDev,
});
...
process.on("uncaughtException", (error) => {
  ...
  captureSupervisorException(error, { "craftstation.feature_area": "supervisor" });
...
process.on("unhandledRejection", (reason) => {
  ...
  captureSupervisorException(reason, { "craftstation.feature_area": "supervisor" });
  void flushSupervisorSentry();
});
