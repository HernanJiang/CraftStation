...
import { ContextMenu, type ContextMenuEntry, PixelLoader } from "@/renderer/components/common";
...
export function ProjectTreeView(props: {
  ...
  /** Collapse only this directory column. The file preview stays put. */
  onCollapseTree?: () => void;
}) {
  const { t } = useLingui();
  ...
  const [rowMenu, setRowMenu] = useState<{
    ...
    request: { x: number; y: number; nonce: number };
  } | null>(null);
  ...
  function openEntryMenu(
    ...
    setRowMenu({
      items,
      entry,
      request: { x: event.clientX, y: event.clientY, nonce: menuNonce.current },
    });
  }
  ...
  return (
    <>
                ...
                <Tooltip delay={300}>
                  <Tooltip.Trigger>
                    <button
                      ...
                      aria-label={t`Hide directory`}
                    >
                      <PanelRightClose className="size-3.5" />
                    </button>
                  </Tooltip.Trigger>
                  <Tooltip.Content placement="bottom">
                    <Trans>Hide directory</Trans>
                  </Tooltip.Content>
                </Tooltip>
              </div>
            ) : null}
          ...
          {tree.draft?.mode === "create" && tree.draft.parentPath === "" ? (
            <div className="px-2 pb-1">
              <InlineDraftRow
                ...
                onChange={(value) =>
                  tree.setDraft((state) => (state ? { ...state, value } : state))
                }
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden" style={scrollFadeStyle}>
            {tree.searchQuery ? (
                ...
                {tree.searchLoading ? (
                  ...
                  tree.searchResults.map((entry) => (
                    <TreeEntryRow
                      ...
                      onOpenContextMenu={(event, items) => openEntryMenu(event, entry, items)}
              ...
              <div
                ...
                className="h-full overflow-y-auto overflow-x-hidden px-1"
              >
                      ...
                      return (
                        ...
                        >
                          <ProjectTreeVirtualRow
                            ...
                            onOpenContextMenu={openEntryMenu}
                            onMovePath={tree.handleMovePath}
                            onHandleRename={tree.handleRenameEntry}
                            onHandleCreate={tree.handleCreateEntry}
                          />
      ...
      {rowMenu ? (
        <ContextMenu
          items={rowMenu.items}
          openRequest={rowMenu.request}
          onAction={(action) => {
            void tree.handleEntryAction(rowMenu.entry, action);
          }}
        >
...
function ProjectTreeVirtualRow(props: {
  ...
  return (
    <TreeEntryRow
      ...
      onOpenContextMenu={(event, items) => rowProps.onOpenContextMenu(event, row.entry, items)}
