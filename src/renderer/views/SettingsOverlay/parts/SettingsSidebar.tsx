...
export function SettingsSidebar(props: {
  ...
  }, [settingsFocusToken, isCollapsed]);
    ...
    <div className={`relative h-full ${overlaySidebarSurfaceClass}`}>
      {isCollapsed && (
        <div className="absolute inset-0 z-10 flex h-full min-h-0 flex-col items-start gap-3 pl-2 pb-1 pt-0">
          ...
          <div className="border-t border-[var(--hairline)] pb-2 pt-2">
            <button
              type="button"
              aria-label="CraftStation"
              onClick={onClose}
              className="flex size-8 items-center justify-center rounded-xl hover:bg-[var(--row-hover)]"
            >
              <img src={brandLogoUrl} alt="" className="size-5 rounded-md object-contain" />
            </button>
          </div>
        </div>
      )}
      ...
      >
        <div className={sidebarBodyScrollClass()}>
          ...
          <label
            data-craftstation-find-scope="settings"
        ...
        <div className="flex shrink-0 items-center gap-1 border-t border-[var(--hairline)] py-2">
          <button
            type="button"
            onClick={onClose}
            className="flex min-w-0 flex-1 items-center gap-2 rounded-xl px-2 py-1.5 text-left transition-colors hover:bg-[var(--row-hover)]"
          >
            <img src={brandLogoUrl} alt="" className="size-5 rounded-md object-contain" />
            <span className="truncate text-xs font-semibold text-foreground">CraftStation</span>
          </button>
          <button
            type="button"
          ...
          <Dropdown>
            ...
            <Dropdown.Popover placement="top end" className="min-w-[230px] rounded-[14px]">
