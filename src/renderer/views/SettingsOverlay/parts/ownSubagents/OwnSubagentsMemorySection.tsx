 ...
 */
export function OwnSubagentsMemorySection() {
  const { t } = useLingui();
  const entries = useSharedSettings((s) => s.ownSubagentSelectionUsage);
  ...
  async function removeEntry(entry: CrossagentSelectionUsageEntry) {
    ...
    setPendingKey(key);
    try {
      const next = await readBridge().removeOwnSubagentsMemoryEntry({
        entry: crossagentSelectionUsageEntryKey(entry),
      });
      useSharedSettings.setState({ ownSubagentSelectionUsage: next });
    } catch {
  ...
  async function retagEntry(entry: CrossagentSelectionUsageEntry, tags: string[]) {
    ...
    setPendingKey(key);
    try {
      const next = await readBridge().updateOwnSubagentsMemoryEntryTags({
        entry: crossagentSelectionUsageEntryKey(entry),
        tags,
      });
      useSharedSettings.setState({ ownSubagentSelectionUsage: next });
