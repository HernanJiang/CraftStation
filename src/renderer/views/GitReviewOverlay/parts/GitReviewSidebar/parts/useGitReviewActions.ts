...
export function useGitReviewActions(args: UseGitReviewActionsArgs) {
  ...
  async function handleCommit(
    ...
    try {
      ...
      const aiGen =
        autoGeneratedMessage && commitGenResult
          ? commitGenResult
          : commitGen && commitGen.text.trim() === message
            ? commitGen
            : undefined;
      if (aiGen) {
        recordAiAction("commit", aiGen.provider, aiGen.model);
      }
      ...
      if (pushAfter && hasRemote) {
        ...
        try {
          ...
          // The push belongs to the same AI-assisted flow as the commit above:
          // only attribute it when the commit itself was AI-attributed.
          if (aiGen) {
            recordAiAction("push", aiGen.provider, aiGen.model);
          }
          refreshPrAfterPush();
