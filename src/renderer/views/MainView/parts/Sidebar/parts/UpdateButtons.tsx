...
export function useUpdateEntryVisible(): boolean {
  return useUpdateStore(
    (s) =>
      s.phase === "downloading" ||
      s.phase === "downloaded" ||
      Object.keys(s.agentUpdates).length > 0,
  );
}
...
export function UpdateButtons(props: {
  ...
  const agentEntries = Object.values(agentUpdates);

  const showAppDownload = updatePhase === "downloading";
  const showInstall = updatePhase === "downloaded";
  if (!showAppDownload && !showInstall && agentEntries.length === 0) {
    return null;
  }

  // Agent binary updaters (npm scripts, vendor updaters) stream no byte
  // counts: render an honest indeterminate row per in-flight agent instead of
  // a fabricated percentage.
  const agentRows = agentEntries.map((entry) => (
    <div
      key={entry.label}
      className="flex w-full items-center gap-2 rounded-3xl px-2 py-1.5 text-muted"
    >
      <Download className="size-4 shrink-0 animate-pulse text-muted" />
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="truncate text-xs tabular-nums">
          <Trans>Updating {entry.label}…</Trans>
        </div>
        <div className="h-1 w-full overflow-hidden rounded-full bg-[var(--row-active)]">
          <div className="h-1 w-1/3 animate-pulse rounded-full bg-foreground" />
        </div>
      </div>
    </div>
  ));
  if (iconOnly) {
    const summary = showAppDownload
      ? `${t`Downloading update`} — ${Math.min(100, Math.max(0, Math.round(downloadPercent)))}%`
      : agentEntries.length > 0
        ? agentEntries.map((entry) => entry.label).join(", ")
        : updateVersion
          ? t`Install v${updateVersion}`
          : t`Install update`;
    return (
      <Tooltip delay={150}>
        <Tooltip.Trigger>
