 ...
 * grouping; their headers carry the project tag for their children. New-thread
 * creation is intentionally centralized in the sidebar's top action menu, so
 * this list no longer renders a duplicate creation row. Sorting follows the
 * shared sort mode, except per-project manual order, which has no meaning across
 * projects and falls back to last-updated.
 */
export function SidebarFlatThreadList(props: { sortMode: ThreadSortMode }) {
  ...
  // Own scroll container (the grouped/empty bodies use Sidebar's): the project
  // filter above it stays pinned while the rows scroll.
