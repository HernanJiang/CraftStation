...
describe("SidebarFlatThreadList", () => {
  ...
  it("shows Home threads alongside project threads without a duplicate new-thread row", () => {
  ...
  it("filters threads to the selected projects without adding a creation row", () => {
  ...
  it("keeps an empty filtered list free of duplicate creation controls", () => {
    useSidebarUiStore.setState({ flatListProjectFilter: ["local-1"] });
    useAppStore.setState({
      projects: [homeProject, localProject],
  ...
  it("keeps only the project filter above rows when several projects are visible", () => {
    useAppStore.setState({
      projects: [homeProject, localProject],
      threads: [makeThread("p1", "local-1", "2026-08-01T10:00:00.000Z")],
    });
  ...
  it("pins the project filter above the scrolling rows", () => {
    useAppStore.setState({
      projects: [homeProject, localProject],
      threads: [makeThread("p1", "local-1", "2026-08-01T10:00:00.000Z")],
    });
    ...
    const head = screen.getByTestId("project-filter");
  ...
  it("renders no filter or duplicate creation row when only one project is visible", () => {
    useSharedSettings.setState({ homeScopeEnabled: false } as never);
    useAppStore.setState({
      projects: [homeProject, localProject],
      threads: [makeThread("p1", "local-1", "2026-08-01T10:00:00.000Z")],
