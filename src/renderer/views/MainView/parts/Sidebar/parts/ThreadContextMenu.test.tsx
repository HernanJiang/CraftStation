...
function renderMenu(
  ...
  options: { showProjectActions?: boolean; onRename?: () => void } = {},
) {
  render(
    <ThreadContextMenu
      thread={target}
      project={owner}
      {...(options.showProjectActions !== undefined
        ? { showProjectActions: options.showProjectActions }
        : {})}
  ...
  const contextMenuEventAccepted = fireEvent.contextMenu(
    screen.getByRole("button", { name: "row" }),
  );
  expect(contextMenuEventAccepted).toBe(false);
...
describe("ThreadContextMenu project actions", () => {
  ...
  it("keeps the core thread actions in the shared right-click menu", async () => {
