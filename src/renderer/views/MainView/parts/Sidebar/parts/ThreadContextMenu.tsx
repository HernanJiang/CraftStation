...
export function ThreadContextMenu(props: {
  ...
  openRequest?: ContextMenuOpenRequest | null;
  children: ReactNode;
}) {
  const { thread, project, onRename } = props;
  const { t } = useLingui();
  const [detailsView, setDetailsView] = useState<"thread" | "branch" | null>(null);
    ...
    <>
      <ContextMenu
        ...
        items={[
          ...
          {
            id: "git-branch-info",
            label: t`Git branch information`,
        ...
        onAction={(key, anchorPosition, returnFocusElement) => {
          if (key === "thread-details") {
            setDetailsView("thread");
          }
          if (key === "git-branch-info") {
            setDetailsView("branch");
          }
      ...
      >
        <Modal.Container placement="center" size="sm">
          <Modal.Dialog className="sm:max-w-md">
            ...
            <Modal.Header>
              <Modal.Heading>
                {detailsView === "branch" ? t`Git branch information` : t`Thread details`}
              </Modal.Heading>
            </Modal.Header>
            <Modal.Body className="space-y-2 px-5 pb-5 text-xs">
              <ThreadDetailRow label={t`Name`} value={thread.title} />
              <ThreadDetailRow label={t`Project`} value={project.name} />
              <ThreadDetailRow
                label={t`Git branch`}
                value={thread.worktreeBranch ?? t`Project default branch`}
              />
              {detailsView === "thread" ? (
                <>
                  <ThreadDetailRow label={t`Status`} value={thread.status} />
                  <ThreadDetailRow label={t`Provider`} value={thread.agentKind} />
                  <ThreadDetailRow label={t`Updated`} value={thread.updatedAt} />
                  <ThreadDetailRow label={t`Thread ID`} value={thread.id} mono />
                </>
              ) : null}
            </Modal.Body>
          </Modal.Dialog>
...
function ThreadDetailRow(props: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 rounded-lg bg-white/[0.035] px-3 py-2">
      <span className="text-muted">{props.label}</span>
      <span className={`min-w-0 break-all text-foreground ${props.mono ? "font-mono" : ""}`}>
        {props.value}
      </span>
    </div>
  );
