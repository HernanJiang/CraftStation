...
export function AppShell(props: {
  ...
  balanceRightPanelOnOpen?: boolean;
  balanceBottomPanelOnOpen?: boolean;
  forceSidebarExpanded?: boolean;
  onRequestClosePanels?: () => void;
  onDismissRightOverlay?: () => void;
}) {
  const { t } = useLingui();
  const { sidebar, content, globalHeader, sidebarHeader, contentHeader, rightPanel, gitPanel } =
    props;
  ...
  const auxiliaryPanelMaximized = usePanelStore(
    (state) => state.auxiliaryPanelMaximized && state.auxiliaryPanelPlacement !== "hidden",
  );
  ...
  const hasHeaders = globalHeader != null || sidebarHeader != null || contentHeader != null;
  const hasContentHeader = contentHeader != null;

  // Codex opens its inspector as a peer workspace rather than a narrow utility
  // drawer. On each explicit right-dock open, start both panes at an even split;
  ...
  useLayoutEffect(() => {
    ...
    // Keep a small safety margin on the main side so the equal split does not
    // land exactly on CONTENT_MIN_WIDTH after borders/sub-pixel rounding and
    // trip the shell's narrow-window overlay auto-hide path.
    updatePanelWidth(availableWidth / 2 - 8);
  }, [isBottom, props.balanceRightPanelOnOpen, rightPanelOpen, updatePanelWidth]);

  useLayoutEffect(() => {
    if (!props.balanceBottomPanelOnOpen || !rightPanelOpen || !isBottom) return;
    const host = mainRef.current?.parentElement;
    if (!host) return;
  ...
  const rightOverlayTop = globalHeader
    ? "38px"
    : hasContentHeader
  ...
  const workspaceContentHeader = contentHeader ? (
    <div
      ...
      style={{
        ...
        // titlebar-area height because they do not mount CraftStation's 38px
        // global titlebar.
        height: globalHeader ? "38px" : "env(titlebar-area-height, 32px)",
        paddingRight: globalHeader
          ? "12px"
          : isWindows()
  ...
  const rightPanelSlot = rightPanel ? (
    <AsideSlot
