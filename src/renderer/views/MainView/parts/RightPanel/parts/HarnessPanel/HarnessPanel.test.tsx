...
describe("HarnessPanel native control-plane surface", () => {
  ...
  it("fails closed without selectable OpenAI models when official discovery is unavailable", async () => {
    bridgeMock.getCraftingModelInventory.mockResolvedValue({
      status: "unavailable",
      source: "codex-app-server-model-list",
      models: [],
    ...
    const gridProps = mockCraftingGrid.mock.calls.at(-1)?.[0] as { models: Item[] };
    expect(gridProps.models.some((item) => item.metadata.vendor === "openai")).toBe(false);
  });
