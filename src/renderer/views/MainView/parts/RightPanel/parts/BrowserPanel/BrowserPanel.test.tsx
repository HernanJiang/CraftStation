...
describe("BrowserPanel", () => {
  ...
  it("renders a <webview> per tab and hides inactive ones", () => {
    ...
    expect(webviews[0]?.getAttribute("partition")).toBe("persist:craftstation-browser");
  ...
  it("hides the nested browser tab strip when tabs live in the auxiliary top bar", () => {
    useBrowserPanelStore.setState({
      tabs: [
        {
          tabId: "tab-1",
    ...
    const { queryByTestId } = render(<BrowserPanel visible showEmbeddedTabStrip={false} />);

    expect(queryByTestId("browser-tab-strip")).toBeNull();
    expect(queryByTestId("browser-toolbar")).toBeTruthy();
  });
