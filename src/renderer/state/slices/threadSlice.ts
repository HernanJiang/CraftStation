...
export interface ThreadSlice {
  ...
  markThreadDone: (threadId: string, options?: { preservePane?: boolean }) => void;
...
export const createThreadSlice: SliceCreator<ThreadSlice> = (set) => ({
  ...
  markThreadDone: (threadId, options) =>
    set((state) => {
      ...
      let nextView = state.view;
      if (!options?.preservePane && state.view.kind === "thread") {
        nextView = removePaneFromView(state.view, threadId);
      }

      // Sidebar/context-menu completion closes the pane and drops terminal
      // keep-alive state. The in-pane header action is deliberately different:
      // it marks the conversation done in place, leaving X as the only control
      // that removes the active conversation from the workspace.
      const keepAlivePaneIds = options?.preservePane
        ? state.keepAlivePaneIds
        : removeKeepAliveId(state.keepAlivePaneIds, threadId);
      return { threads, view: nextView, keepAlivePaneIds };
    }),
