...
function IngredientSlotView(props: {
  ...
  return (
    <div
      ...
      data-crafting-slot=""
      title={props.item ? [props.item.metadata.name, ...componentLines(props.item)].join("\n") : label}
      onDragOver={(event) => {
...
export const CraftingGrid: React.FC<CraftingGridProps> = ({ workspace, onCraft, disabled = false }) => {
  ...
  const [selectedModelId, setSelectedModelId] = useState(availableModels[0]?.id ?? "openai:gpt-5.3-codex");
  ...
  const selectedHarnessItem: SlotSelection = harnessSelection === "auto" ? "auto" : registry.getItem(harnessSelection);
  const resolvedHarness = registry.resolveSlot("harness", selectedHarnessItem);
  const validation = crafter.validate({ slots: { model: selectedModel, harness: selectedHarnessItem } });

  const visibleItems = [...availableModels, ...availableHarnesses].filter((item) =>
    filter === "all" ? true : filter === "model" || filter === "harness" ? item.kind === filter : false,
  );
  ...
  async function handleCraft() {
    ...
    try {
      ...
      const result = crafter.compile({ slots: { model: selectedModel, harness: selectedHarnessItem } }, context);
      if (!result.success) {
        setErrorMessage(result.errors?.map((error) => error.message).join("; ") ?? "Crafting compilation failed");
        return;
      }
      if (onCraft) await onCraft(result, prompt);
      else console.info("[Feature Pending Implementation] Spawn crafted Agent Entity", result.craftPlan?.id);
    } catch (error) {
  ...
  return (
    <div data-testid="crafting-grid" className="flex min-h-0 w-full flex-col gap-3 rounded-[20px] border border-[var(--hairline)] bg-[var(--surface)] p-3 text-foreground">
      <header className="flex items-center justify-between gap-3">
        ...
        <span className="rounded-full bg-[var(--surface-secondary)] px-2 py-1 font-mono text-[9px] text-muted">3×3 GRID</span>
      </header>
      ...
      <div className="grid grid-cols-[172px_24px_minmax(88px,1fr)] items-center justify-center gap-3 rounded-[18px] bg-[var(--content-background)] p-3">
        <div className="grid grid-cols-3 gap-2">
          ...
          />
          <div data-crafting-slot="" className="craftstation-mc-slot flex size-[52px] items-center justify-center rounded-lg border border-white/10 text-muted/25">
          ...
          <span className="sr-only">
            Resolved: {resolvedHarness?.metadata.name ?? "None"}
          </span>
        ...
        >
          {validation.valid && validation.matchedRecipe ? (
            <>
              ...
              <span className="sr-only">Produces: {selectedModel?.metadata.name} + {resolvedHarness?.metadata.name}</span>
      ...
      <section className="min-h-0 rounded-[18px] border border-[var(--hairline)] bg-[var(--content-background)] p-3">
        <div className="no-scrollbar flex gap-1 overflow-x-auto pb-2">
          {FILTERS.map((entry) => {
            ...
            return <button key={entry.id} type="button" onClick={() => setFilter(entry.id)} className={`flex h-7 shrink-0 items-center gap-1.5 rounded-lg px-2 text-[10px] transition-colors ${filter === entry.id ? "bg-[var(--row-active)] text-foreground" : "text-muted hover:bg-[var(--row-hover)]"}`}><Icon className="size-3.5" />{entry.label}</button>;
          })}
        </div>
        {filter === "recipe" ? (
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {recipes.map((recipe) => <button key={recipe.id} type="button" title={recipe.description} onClick={() => { if (availableModels[0]) setSelectedModelId(availableModels[0].id); setHarnessSelection("auto"); }} className="rounded-xl border border-[var(--hairline)] bg-[var(--surface-secondary)] p-3 text-left hover:bg-[var(--row-hover)]"><ScrollText className="size-4 text-amber-300" /><span className="mt-2 block text-xs font-semibold">{recipe.name}</span><span className="text-[9px] text-muted">一键填充 Ingredients · {recipe.compatibilityStatus}</span></button>)}
          </div>
        ) : filter === "component" ? (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {[...availableModels, ...availableHarnesses].flatMap((item) => item.components.map((component, index) => ({ id: `${item.id}:${index}`, name: component.kind, owner: item.metadata.name }))).map((component) => <div key={component.id} title={`${component.name}\nOwner: ${component.owner}`} className="rounded-xl border border-[var(--hairline)] bg-[var(--surface-secondary)] p-2"><Puzzle className="size-4 text-violet-300" /><span className="mt-2 block truncate text-[10px] font-semibold">{component.name}</span><span className="block truncate text-[9px] text-muted">{component.owner}</span></div>)}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
            {visibleItems.map((item) => <InventoryItem key={item.id} item={item} selected={item.id === selectedModelId || item.id === resolvedHarness?.id} onSelect={() => chooseItem(item)} />)}
          </div>
        )}
      </section>

      <select data-testid="model-select" className="sr-only" value={selectedModelId} onChange={(event) => setSelectedModelId(event.target.value)}>{availableModels.map((item) => <option key={item.id} value={item.id}>{item.metadata.name} ({item.metadata.vendor})</option>)}</select>
      <select data-testid="harness-select" className="sr-only" value={harnessSelection} onChange={(event) => setHarnessSelection(event.target.value)}><option value="auto">auto（确定性 Codex）</option>{availableHarnesses.map((item) => <option key={item.id} value={item.id}>{item.metadata.name} ({item.metadata.vendor})</option>)}</select>

      <div className="flex flex-col gap-2 sm:flex-row">
        <input id="craft-prompt-input" data-testid="craft-prompt-input" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="初始任务描述（可选）" className="min-w-0 flex-1 rounded-xl border border-[var(--hairline)] bg-[var(--content-background)] px-3 py-2 text-xs outline-none focus:border-amber-400/50" />
        <button type="button" data-testid="craft-button" onClick={() => void handleCraft()} disabled={!validation.valid || disabled || isCrafting} className="flex h-9 items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 text-xs font-semibold text-neutral-950 transition-all hover:bg-amber-400 disabled:cursor-not-allowed disabled:bg-[var(--surface-secondary)] disabled:text-muted"><Sparkles className="size-4" />{isCrafting ? "正在合成 Agent..." : "合成并启动 Agent"}</button>
      </div>
      {errorMessage ? <p className="rounded-xl bg-rose-500/10 px-3 py-2 text-xs text-rose-300">{errorMessage}</p> : null}
    </div>
  );
};
