...
vi.mock("./ThreadComposer", () => ({
  ThreadComposer: (props: {
    ...
    <div>
      ...
      {props.attachmentBar}
      {props.inputContent}
      <output data-testid="composer-variant">{props.variant ?? "active"}</output>
      <output data-testid="composer-density">{props.compact ? "compact" : "full"}</output>
      <output data-testid="composer-controls-display">{props.controlsDisplay ?? "inline"}</output>
      <output data-testid="composer-leading-controls">
        {props.leadingControls ? "present" : "absent"}
      </output>
...
describe("ThreadComposerSection", () => {
  ...
  it("keeps the active-thread composer visually aligned with the new-thread composer", () => {
    const { container } = renderComposer();

    expect(screen.getByTestId("composer-variant")).toHaveTextContent("draft");
    expect(screen.getByTestId("composer-density")).toHaveTextContent("full");
    expect(screen.getByTestId("composer-controls-display")).toHaveTextContent("menu");
    expect(screen.getByTestId("composer-leading-controls")).toHaveTextContent("present");
    expect(
