...
describe("humanIntentTitle", () => {
  ...
  it("describes PowerShell Get-Content -Path ranges", () => {
    const full = String.raw`cd C:\Users\sdsle\work\craftstation && "C:\\Program Files\\WindowsApps\\Microsoft.PowerShell_7.6.1.0_x64__8wekyb3d8bbwe\\pwsh.exe" -Command 'Get-Content -Path src/renderer/components/thread/ChatPane/parts/items/ToolCallGroup.tsx | Select-Object -Skip 550 -First 110'`;
  ...
  it("preserves Windows paths in PowerShell Get-Content -LiteralPath", () => {
    const full = String.raw`pwsh -Command 'Get-Content -LiteralPath C:\Users\sdsle\work\craftstation\src\foo.ts'`;
    const display = commandIntentDisplay(full);

    expect(display.title).toBe("View: foo.ts");
    expect(display.parts).toEqual({
      prefix: "View: ",
      path: String.raw`C:\Users\sdsle\work\craftstation\src\foo.ts`,
      filePath: true,
    });
  });
  ...
  it("strips PowerShell cd …; before intent", () => {
    const full = 'cd "c:\\Users\\me\\work\\craftstation"; pnpm exec oxfmt src/a.ts';
