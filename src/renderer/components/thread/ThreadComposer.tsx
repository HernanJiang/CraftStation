...
import { ArrowUp, ChevronDown, ClipboardList, ShieldCheck, Square, Zap } from "lucide-react";
import { Dropdown, Label, ToggleButton, Tooltip } from "@heroui/react";
...
type PermissionComposerControl =
  | Extract<ComposerControl, { kind: "toggle" }>
  | Extract<ComposerControl, { kind?: "menu" }>;
type ModeComposerControl = Extract<ComposerControl, { kind: "toggle" }>;

function isPermissionComposerControl(
  control: ComposerControl,
): control is PermissionComposerControl {
  return (
...
function isModeComposerControl(control: ComposerControl): control is ModeComposerControl {
...
function permissionOptionIsFull(option: OptionMenuOption): boolean {
  ...
  return (
    id === "never" ||
    id === "full-access" ||
    label.includes("full access") ||
    label.includes("auto approve")
  );
}
...
function permissionIsFull(control: PermissionComposerControl | undefined): boolean {
  ...
  return selected ? permissionOptionIsFull(selected) : control.value === "never";
}

function setPermission(control: PermissionComposerControl | undefined, full: boolean) {
  if (!control) {
    console.info("[Feature Pending Implementation] Permission control is unavailable");
    return;
  }
  if (control.kind === "toggle") {
      ...
      <Dropdown.Popover placement="top start" className="min-w-[210px] rounded-[14px]">
        <Dropdown.Menu
          ...
          onAction={(key) => {
            ...
            if (next === "plan") {
              if (modeControl) modeControl.onChange?.(true);
              else console.info("[Feature Pending Implementation] Plan mode is unavailable");
              return;
            }
            modeControl?.onChange?.(false);
...
export function ThreadComposer(props: {
  ...
  /** Compact product-specific action immediately before the pinned model control. */
  ...
  const renderControlsList = (targetWrapLevel: number, forceShowLabels = false) =>
    controls
      .filter((control) => !(control.kind === "provider-model" && control.alignEnd))
      .map((control, index) => renderControlItem(control, index, targetWrapLevel, forceShowLabels));
  ...
  const renderControls = () => (
    <div className="relative min-w-0 flex-1">
      ...
      <div
        ...
        style={{ height: "2.25rem" }}
      >
        {controlsDisplay === "inline" ? renderControlsList(0) : null}
      </div>
    </div>
  );
    ...
    <div ref={toolbarRef} className={toolbarClassName} data-wrap-level={wrapLevelRef.current}>
      ...
      <div className="flex shrink-0 items-end gap-1">
        ...
        {controlsDisplay === "menu" ? (
          ...
          controls
            .filter((control) => control.kind === "provider-model" && control.alignEnd)
            .map((control, index) => renderControlItem(control, index, 0, false))
        )}
      ...
      {controlsDisplay === "menu"
        ? controls.map((control, index) =>
            control.kind === "provider-model" && control.openSignal !== undefined ? (
              <div key={`command-model-picker-${index}`} className="sr-only">
                {renderControlItem(control, index, 0, false)}
              </div>
            ) : null,
          )
        : null}
