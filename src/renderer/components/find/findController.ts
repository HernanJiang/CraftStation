 ...
 * (cursor inside Monaco, an xterm, or a surface tagged `data-craftstation-find-
...
export function resolveFindTarget(): FindTarget | null {
  ...
  const scope = element
    ?.closest("[data-craftstation-find-scope]")
    ?.getAttribute("data-craftstation-find-scope");
  if (scope === "git" || scope === "settings" || scope === "tree" || scope === "chat") {
    return scope;
  }
  if (element?.closest("[data-craftstation-browser]")) return null;
