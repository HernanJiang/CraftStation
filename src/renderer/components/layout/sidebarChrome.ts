 ...
 * `.craftstation-shell`) adds 28px of left padding so they clear the traffic-light controls. The
 * gate is pure CSS, so descendants don't subscribe to sidebar state.
 */
export const macosTrafficLightPadClass = "craftstation-mac-traffic-light-pad" as const;
...
export const panelHeaderIconButtonClass =
  "inline-flex items-center justify-center rounded-xl p-1 text-muted transition-colors hover:bg-[var(--row-hover)] hover:text-foreground";
...
export function panelHeaderTabIconButtonClass(active: boolean) {
  return `inline-flex items-center justify-center rounded-xl p-1 transition-colors ${
    active
 ...
 * The `craftstation-overlay-surface` marker lets the translucent-sidebar CSS turn this
