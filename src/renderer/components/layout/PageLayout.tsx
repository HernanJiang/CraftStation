  ...
  if (onTitleClick) {
    return (
      <button
        type="button"
        aria-label={title}
        className="craftstation-overlay-header__controls shrink-0 leading-none text-muted transition-colors hover:text-foreground"
            ...
            <Tooltip delay={150}>
              <Tooltip.Trigger>
                <Button
                  ...
                  className="craftstation-overlay-header__controls size-6 min-w-0 shrink-0 text-muted hover:text-foreground"
...
export function PageLayout(props: {
  ...
  hideSidebarHeaderTitle?: boolean;
  sidebar: ReactNode;
  content: ReactNode;
  ...
  const {
    ...
    balanceRightPanelOnOpen,
    balanceBottomPanelOnOpen,
    forceSidebarExpanded,
    onRequestClosePanels,
    onDismissRightOverlay,
  } = props;

  const sidebarHeader = globalHeader ? undefined : (
    <SidebarHeaderRow
      title={title}
      titleNode={titleNode}
      {...(onTitleClick != null ? { onTitleClick } : {})}
      hideTitle={hideSidebarHeaderTitle === true}
    >
      {sidebarHeaderChildren}
    </SidebarHeaderRow>
  );

  // macOS only: drop the empty center `craftstation-overlay-header` when there is no content so main
  ...
  const contentHeader = hideContentHeader
    ? null
    : (contentHeaderChildren ?? (isMac() ? null : <></>));
  ...
  const shell = (
    <AppShell
      ...
      {...(balanceRightPanelOnOpen !== undefined ? { balanceRightPanelOnOpen } : {})}
      {...(balanceBottomPanelOnOpen !== undefined ? { balanceBottomPanelOnOpen } : {})}
