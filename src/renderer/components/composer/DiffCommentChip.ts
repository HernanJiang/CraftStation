...
export function createDiffCommentChipElement(comment: DiffCommentSegment): HTMLSpanElement {
  ...
  chip.className = "craftstation-slash-chip craftstation-diff-comment-chip";
  chip.title = `${diffCommentTarget(comment)}\n${comment.body}`;

  const icon = document.createElement("span");
  icon.className = "craftstation-slash-chip__slash";
  icon.innerHTML =
  ...
  name.className = "craftstation-slash-chip__name";
  name.textContent = diffCommentTarget(comment, true);
  chip.appendChild(name);

  const remove = document.createElement("span");
  remove.className = "craftstation-diff-comment-chip__delete";
  remove.innerHTML =
