...
import { handleThreadStateNotification, shouldInspectThreadStateForNotification, showInAppUserNotification } from '@/renderer/notifications';
...
export const runtimeEventsContribution: WorkbenchContribution<WorkbenchServices> = {
 id: 'runtime.events', phase: 'starting',
...
function handleSupervisorEvent(event: SupervisorEvent): void {
  ...
  if (event.type === "session-switch-state") {
    void applySessionHandoffState(event.state).catch((error: unknown) => services.reportError('session.handoff', error));
    return;
  }
  ...
  if (event.type === "thread-state") {
    const shouldCheckNotifications =
      isReady() && shouldInspectThreadStateForNotification();
    ...
    // orphaned 鈥?its parent `item.completed` will never arrive. Reconcile so
  ...
  if (event.type === "thread-pool-failover") {
    ...
    // history, so no error UI 鈥?one top toast tells who died and who took
    // over. The composer error dock clears itself once the follow-up answer
    // lands (recovered errors are not actionable).
    showTopStatusToast(`${providerLabel(event.provider)}璐﹀彿${event.fromAccount}棰濆害宸茶€楀敖`, {
      description: `宸插垏鎹㈠埌${event.toAccount}缁х画浣滅瓟`,
    });
  }
