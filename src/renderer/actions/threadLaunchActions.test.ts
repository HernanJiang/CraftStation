...
import {
  ...
  startThreadFromCraft,
  startThreadFromDraft,
} from "./threadLaunchActions";
...
describe("startThreadFromDraft host transport", () => {
  beforeEach(() => {
    ...
    mocks.appState.createThread.mockImplementation((input) => {
      ...
      const thread = {
        ...
        ...(values.compositionProvenance
          ? { compositionProvenance: values.compositionProvenance }
          : {}),
    ...
    mocks.bridge.craftAgent.mockResolvedValue({
      threadId: "craft-thread-1",
      entityId: "entity:codex:test",
      sessionId: "sess:codex:craft-thread-1",
      response: "",
    });
  ...
  it("hands a crafted plan to the production craftAgent seam using its plan thread id", async () => {
    const craftResult = new Crafter().compile(
      ...
      { workspace: "C:\\repo", threadId: "craft-thread-1" },
    );

    await startThreadFromCraft(localProject, craftResult, "inspect the repository");
    ...
    expect(mocks.bridge.dbSetState).toHaveBeenCalledWith(
      "craftstation:provenance:craft-thread-1",
      expect.any(String),
    );
    ...
    expect(mocks.bridge.resumeCraftAgent).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        craftPlan: expect.objectContaining({
          threadId: "local-thread",
          sessionRef: "rollout-resume-1",
          runtimeBinding: expect.objectContaining({ modelId: "gpt-5.3-codex" }),
        }),
      }),
    );
    expect(mocks.bridge.startThread).not.toHaveBeenCalled();
