 ...
 */
export function markThreadDone(threadId: string, options?: { preservePane?: boolean }): void {
  if (findExperimentByThreadId(threadId)) return;
  const store = useAppStore.getState();
  ...
  if (
    dispatchRemoteThreadMutation(
      ...
      () => useAppStore.getState().markThreadDone(threadId, options),
    )
  )
    return;
  ...
  if (!options?.preservePane && worktreePath && isLastOpenWorktreeThread) {
  ...
  store.markThreadDone(threadId, options);
}

export function toggleMarkThreadDone(threadId: string, options?: { preservePane?: boolean }): void {
  if (findExperimentByThreadId(threadId)) return;
  const store = useAppStore.getState();
  const thread = store.threads.find((t) => t.id === threadId);
  if (!thread) return;
  ...
  if (
    dispatchRemoteThreadMutation(
      ...
      () => {
        const nextStore = useAppStore.getState();
        if (done) nextStore.markThreadDone(threadId, options);
