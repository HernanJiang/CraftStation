...
const visibleTaskToastIds: Array<Parameters<typeof toast.close>[0]> = [];
const MAX_VISIBLE_TASK_TOASTS = 3;
...
function trackTaskToast(toastId: Parameters<typeof toast.close>[0]): void {
  visibleTaskToastIds.push(toastId);
  while (visibleTaskToastIds.length > MAX_VISIBLE_TASK_TOASTS) {
    const oldestToastId = visibleTaskToastIds.shift();
    if (oldestToastId !== undefined) toast.close(oldestToastId);
  }
}
...
export function showInAppUserNotification(input: {
  threadId: string;
  title: string;
  body: string;
}): void {
