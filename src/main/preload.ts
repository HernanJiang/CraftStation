...
import { type CraftStationChannel, normalizeChannel } from "@/shared/channel";
...
import {
  ...
  IPC_EVENT_CHANNELS,
  IPC_WINDOW_CHANNELS,
  CRAFTSTATION_WINDOW_KINDS,
  type BrowserEvent,
  type CraftStationBridge,
  type CraftStationWindowKind,
 ...
 * built-ins that can fail and drop `window.craftstation` (index.html then redirects
 * to mobile.html).
 */
...
function resolveChannel(): CraftStationChannel {
...
function resolveWindowKind(): CraftStationWindowKind {
  const kind = resolveArgValue("--lc-window-kind=");
  return (CRAFTSTATION_WINDOW_KINDS as readonly string[]).includes(kind)
    ? (kind as CraftStationWindowKind)
    : "main";
}
...
const bridge: CraftStationBridge = {
