...
public static class CraftStationComputerUseNative {
...
try { [void][CraftStationComputerUseNative]::SetProcessDpiAwarenessContext([IntPtr](-4)) } catch {}
...
function Get-WindowObject([IntPtr]$hWnd, [switch]$AllowHidden, [hashtable]$ProcessMap) {
  if (-not [CraftStationComputerUseNative]::IsWindow($hWnd)) { return $null }
  if (-not $AllowHidden -and -not [CraftStationComputerUseNative]::IsWindowVisible($hWnd)) { return $null }
  $titleBuilder = [Text.StringBuilder]::new(512)
  [void][CraftStationComputerUseNative]::GetWindowText($hWnd, $titleBuilder, $titleBuilder.Capacity)
  $title = $titleBuilder.ToString()
  if ($title.Trim().Length -eq 0) { return $null }
  $procId = [uint32]0
  [void][CraftStationComputerUseNative]::GetWindowThreadProcessId($hWnd, [ref]$procId)
  ...
  $rect = New-Object CraftStationComputerUseNative+RECT
  [void][CraftStationComputerUseNative]::GetWindowRect($hWnd, [ref]$rect)
...
function Get-WindowList {
  ...
  foreach ($hWnd in [CraftStationComputerUseNative]::Windows()) {
...
function Recover-Window($req) {
  ...
  for ($recover = 0; $recover -lt 8; $recover++) {
    ...
    if ($matches.Count -gt 0) {
      ...
      $fg = [CraftStationComputerUseNative]::GetForegroundWindow()
...
function Try-SetForeground([IntPtr]$hWnd) {
  [void][CraftStationComputerUseNative]::AllowSetForegroundWindow($ASFW_ANY)
  $fg = [CraftStationComputerUseNative]::GetForegroundWindow()
  $fgPid = [uint32]0
  $fgThread = [CraftStationComputerUseNative]::GetWindowThreadProcessId($fg, [ref]$fgPid)
  $cur = [CraftStationComputerUseNative]::GetCurrentThreadId()
  $attached = $false
  if ($fgThread -ne $cur) { $attached = [CraftStationComputerUseNative]::AttachThreadInput($fgThread, $cur, $true) }
  try {
    [void][CraftStationComputerUseNative]::BringWindowToTop($hWnd)
    [void][CraftStationComputerUseNative]::SetForegroundWindow($hWnd)
  } finally {
    if ($attached) { [void][CraftStationComputerUseNative]::AttachThreadInput($fgThread, $cur, $false) }
  }
}
...
function Find-WindowByProcessId([uint32]$targetPid, [int64]$preferId) {
  $fallback = $null
  $fg = [CraftStationComputerUseNative]::GetForegroundWindow()
  foreach ($candidateHwnd in [CraftStationComputerUseNative]::Windows()) {
    $wPid = [uint32]0
    [void][CraftStationComputerUseNative]::GetWindowThreadProcessId($candidateHwnd, [ref]$wPid)
    ...
    if ($null -eq $fallback -and [CraftStationComputerUseNative]::IsWindowVisible($candidateHwnd)) {
      $fallback = $candidate
    }
  }
  return $fallback
...
function Activate-Window($window) {
  ...
  $ownerPid = [uint32]0
  [void][CraftStationComputerUseNative]::GetWindowThreadProcessId($hWnd, [ref]$ownerPid)
  ...
  if ([CraftStationComputerUseNative]::IsIconic($hWnd)) {
    [void][CraftStationComputerUseNative]::ShowWindow($hWnd, 9)
    Start-Sleep -Milliseconds 40
  } else {
    [void][CraftStationComputerUseNative]::ShowWindow($hWnd, 5)
  }
  $activated = $false
  $usedAlt = $false
  for ($attempt = 0; $attempt -lt 3; $attempt++) {
    ...
    if (-not [CraftStationComputerUseNative]::IsWindow($hWnd)) {
      $replacement = Find-WindowByProcessId $ownerPid ([int64]$window.id)
      if ($null -eq $replacement) { break }
      $hWnd = [IntPtr]([int64]$replacement.id)
      $window = $replacement
    }
    if ([CraftStationComputerUseNative]::GetForegroundWindow() -eq $hWnd) { $activated = $true; break }
    ...
    [CraftStationComputerUseNative]::keybd_event(0x12, 0, 1, [UIntPtr]::Zero)
    [CraftStationComputerUseNative]::keybd_event(0x12, 0, 3, [UIntPtr]::Zero)
    Try-SetForeground $hWnd
    Start-Sleep -Milliseconds 60
    if ([CraftStationComputerUseNative]::GetForegroundWindow() -eq $hWnd) { $activated = $true; break }
  }
  ...
  if (-not [CraftStationComputerUseNative]::IsWindow($hWnd) -or -not $activated) {
    $replacement = Find-WindowByProcessId $ownerPid ([int64]$window.id)
    if ($null -ne $replacement) {
      $hWnd = [IntPtr]([int64]$replacement.id)
      $window = $replacement
      if ([CraftStationComputerUseNative]::GetForegroundWindow() -eq $hWnd) {
        $activated = $true
      } elseif (-not $activated) {
        Try-SetForeground $hWnd
        Start-Sleep -Milliseconds 60
        if ([CraftStationComputerUseNative]::GetForegroundWindow() -eq $hWnd) { $activated = $true }
      }
    }
  }
  if (-not $activated) {
  ...
  if ($usedAlt) {
    ...
    [CraftStationComputerUseNative]::Key(0x1B, $false)
    [CraftStationComputerUseNative]::Key(0x1B, $true)
    Start-Sleep -Milliseconds 40
  }
  ...
  $fresh = $null
  if ([CraftStationComputerUseNative]::IsWindow($hWnd)) {
...
function Capture-Window($window, $maxDimension, $format) {
  ...
  try {
    $hdc = $graphics.GetHdc()
    try {
      $ok = [CraftStationComputerUseNative]::PrintWindow($hWnd, $hdc, 2)
    } finally {
...
function Resolve-Key($token) {
  ...
  if ($raw.Length -eq 1) {
    ...
    $vk = [CraftStationComputerUseNative]::VkKeyScan([char]$raw[0])
...
function Press-Chord($key) {
  ...
  try {
    foreach ($vk in $modVks) { [CraftStationComputerUseNative]::Key($vk, $false); $pressed.Add($vk) }
    foreach ($vk in $baseVks) { [CraftStationComputerUseNative]::Key($vk, $false); $pressed.Add($vk) }
    for ($i = $baseVks.Count - 1; $i -ge 0; $i--) {
      [CraftStationComputerUseNative]::Key($baseVks[$i], $true); [void]$pressed.Remove($baseVks[$i])
    }
    for ($i = $modVks.Count - 1; $i -ge 0; $i--) {
      [CraftStationComputerUseNative]::Key($modVks[$i], $true); [void]$pressed.Remove($modVks[$i])
    }
  } finally {
    # Never leave a key physically down system-wide if we threw mid-sequence.
    for ($i = $pressed.Count - 1; $i -ge 0; $i--) {
      try { [CraftStationComputerUseNative]::Key($pressed[$i], $true) } catch {}
    }
  }
}
...
function Mouse-Click($button, $count) {
  ...
  if ($b -eq "right" -or $b -eq "r") {
    $down = [CraftStationComputerUseNative]::MOUSEEVENTF_RIGHTDOWN
    $up = [CraftStationComputerUseNative]::MOUSEEVENTF_RIGHTUP
  } elseif ($b -eq "middle" -or $b -eq "m") {
    $down = [CraftStationComputerUseNative]::MOUSEEVENTF_MIDDLEDOWN
    $up = [CraftStationComputerUseNative]::MOUSEEVENTF_MIDDLEUP
  }
  for ($i = 0; $i -lt [Math]::Max(1, [int]$count); $i++) {
    [CraftStationComputerUseNative]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
    Start-Sleep -Milliseconds 20
    [CraftStationComputerUseNative]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
  }
}
...
switch ([string]$request.action) {
  ...
  "get_window_state" {
    ...
    if ($request.input.include_text -eq $true) {
      ...
      $notes += "Detailed UI Automation text is not available in this CraftStation helper yet."
    }
  ...
  "type_text" {
    $window = Require-Window $request.input.window
    $window = Activate-Window $window
    [CraftStationComputerUseNative]::SendUnicodeText([string]$request.input.text)
    $result = [pscustomobject]@{
      ok = $true
      mode = "interactive"
      window = Select-Window $window
  ...
  "launch_app" {
    ...
    function Find-LaunchedWindow([uint32]$targetPid) {
      foreach ($hWnd in [CraftStationComputerUseNative]::Windows()) {
        ...
        $wPid = [uint32]0
        [void][CraftStationComputerUseNative]::GetWindowThreadProcessId($hWnd, [ref]$wPid)
...
function ensureWindowsHelperScript(): string {
  ...
  const dir = join(tmpdir(), "craftstation-computer-use");
