...
function ensureBaseDirectories(paths: CraftStationPaths): void {
...
export function prepareCraftStationDataRoot(
  baseDir?: string,
  ...
  try {
    ...
    if (result.status === "migrated") {
      console.info(`[migrate] imported all available CraftStation data into ${paths.baseDir}`);
    }
  } catch (error) {
    // The source remains untouched and any existing CraftStation directory is
    ...
    console.warn(`[migrate] failed to import CraftStation data into ${paths.baseDir}:`, error);
  }
