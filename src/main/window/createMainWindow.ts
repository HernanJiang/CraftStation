...
export function createMainWindow(options: CreateMainWindowOptions): BrowserWindow {
  ...
  const backgroundColor = isDark ? "#111111" : "#f1f1f4";
  ...
  const windowIconPath = resolveWindowIconPath(options.channel);
  const windowIcon = windowIconPath ? nativeImage.createFromPath(windowIconPath) : undefined;
  const brandedWindowIcon = windowIcon && !windowIcon.isEmpty() ? windowIcon : undefined;
  const window = new BrowserWindow({
    title: options.title,
    ...(brandedWindowIcon ? { icon: brandedWindowIcon } : {}),
    show: false,
  ...
  window.once("ready-to-show", () => {
    // Unpackaged Electron can briefly fall back to electron.exe's resource
    // icon on Windows. Re-apply the branded NativeImage after Chromium has
    // created the native HWND so the taskbar receives CraftStation's glyph.
    if (process.platform === "win32" && brandedWindowIcon) {
      window.setIcon(brandedWindowIcon);
    }
