...
export class ChromeBridgeServer {
  ...
  async start(): Promise<ChromeBridgeInfo> {
    ...
    console.log(
      `[craftstation] Chrome bridge listening on ws://127.0.0.1:${port} — pairing file: ${this.options.pairingFilePath}`,
    );
    return this.info;
  }
  ...
  private writePairingFile(info: ChromeBridgeInfo): void {
    try {
      ...
      // eslint-disable-next-line no-console
      console.error("[craftstation] failed to write Chrome bridge pairing file:", err);
    }
  }
}
