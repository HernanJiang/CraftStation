  ...
  it("maps image attachments to local image content blocks", () => {
    expect(
    ...
    ).toEqual([
      {
        kind: "image",
        mimeType: "image/png",
        dataUrl: "craftstation-local://local/C:/tmp/shot.png",
        path: "C:\\tmp\\shot.png",
        name: "shot.png",
        source: "attachment",
      },
...
describe("toLocalFileUrl", () => {
  it("builds a constant-host URL for a POSIX absolute path", () => {
    expect(toLocalFileUrl("/Users/me/img.png")).toBe("craftstation-local://local/Users/me/img.png");
  });

  it("builds a constant-host URL for a Windows drive path", () => {
    expect(toLocalFileUrl("C:\\Users\\me\\img.png")).toBe(
      "craftstation-local://local/C:/Users/me/img.png",
    );
  });
  ...
  it("percent-encodes path segments that contain literal percent signs", () => {
    ...
    expect(toLocalFileUrl(path)).toBe(
      "craftstation-local://local/C:/Users/me/.grok/sessions/E%253A%255Cwork%255Crepo/assets/shot.png",
    );
  });
  ...
  // as the (lowercased) host. The old `craftstation-local:///<path>` form
  ...
  function resolveLikeProtocolHandler(url: string, platform: "darwin" | "win32"): string {
    // craftstation-local is non-special in Node; swap to a special scheme to
    // reproduce Chromium's standard-scheme canonicalization (host extraction).
    const asSpecial = url.replace(/^craftstation-local:/, "https:");
