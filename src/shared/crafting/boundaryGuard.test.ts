...
describe("Deep Module Boundary Guards", () => {
  ...
  it("ensures nativeCodex runtime has no dependencies on legacy CraftStation ThreadSessionManager or SpawnPipeline", () => {
    const nativeCodexDir = path.resolve(__dirname, "../../supervisor/runtime/nativeCodex");
    const files = getFilesRecursively(nativeCodexDir);

    expect(files.length).toBeGreaterThan(0);

    const legacyCraftStationPatterns = [
    ...
    for (const file of files) {
      const content = fs.readFileSync(file, "utf8");
      for (const pattern of legacyCraftStationPatterns) {
        expect(
