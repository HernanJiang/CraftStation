...
describe("RemoteDesktopClient", () => {
  ...
  it("falls back to the legacy environment endpoint when the CraftStation endpoint is unavailable", async () => {
    const requestedPaths: string[] = [];
    const client = new RemoteDesktopClient("http://127.0.0.1:38987/", undefined, async (url) => {
      const pathname = new URL(url).pathname;
      requestedPaths.push(pathname);
      if (pathname === "/.well-known/craftstation/environment") {
        return new Response(
      ...
      return descriptorResponse(CRAFTSTATION_REMOTE_PROTOCOL_VERSION, ["session:read"]);
    });

    await expect(client.environment()).resolves.toMatchObject({ desktopId: "desktop-1" });
    expect(requestedPaths).toEqual([
      "/.well-known/craftstation/environment",
      "/.well-known/craftstation/environment",
    ]);
  });
