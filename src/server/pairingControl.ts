...
export async function requestPairingFromRunningServer(
  ...
  if (pid === null) {
    throw new Error(`No running CraftStation server found in ${baseDir}.`);
  }
  if (!isProcessAlive(pid)) {
    throw new Error(`CraftStation server process ${pid} is not running.`);
  }
  ...
  try {
    ...
    throw new Error("Timed out waiting for the CraftStation server pairing response.");
  } finally {
    removeFile(requestPath);
    removeFile(responsePath);
