 ...
 *
 * Precedence:
 *   1. `CRAFTSTATION_SECRET_STORAGE_KEY` env (base64, 32 bytes) — lets an operator
...
export function readOrCreateHeadlessSecretKey(baseDir: string): string {
  return readOrCreatePersistedSecret(keyFilePath(baseDir), {
    fromEnv: process.env.CRAFTSTATION_SECRET_STORAGE_KEY?.trim(),
    validateEnv: (value) => {
      if (!isValidKey(value)) {
        throw new Error("CRAFTSTATION_SECRET_STORAGE_KEY must be a base64-encoded 32-byte key.");
      }
    },
    isValid: isValidKey,
 ...
 * via `CRAFTSTATION_REMOTE_RELAY_SECRET`. Unlike the storage key this need not be
...
export function readOrCreateRelaySecret(baseDir: string): string {
  return readOrCreatePersistedSecret(join(baseDir, RELAY_SECRET_FILE), {
    fromEnv: process.env.CRAFTSTATION_REMOTE_RELAY_SECRET?.trim(),
