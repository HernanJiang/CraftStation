...
/**
 * Self-hostable relay. A CraftStation server dials `/host` and registers a server
 ...
 * device and the CraftStation server, and the relay only binds a serverId to the
...
function normalizePublicBaseUrl(raw: string): string {
  ...
  try {
    ...
    throw new Error("CRAFTSTATION_RELAY_PUBLIC_BASE_URL must be an absolute http(s) URL.");
  }
