...
- Settings: JSON at `~/.craftstation/settings.json`; renderer store `sharedSettingsStore.ts`; schema `src/shared/settings.ts`. At design time: no last-used directory persisted, no create-directory IPC.
