...
- New extension target `CraftStationActivities`. The `ios/` project is not
  committed (it's generated via `npx cap add`), so the extension's Swift
  sources live in-repo under `native/ios/CraftStationActivities/`;
...
### 2. Capacitor plugin `@craftstation/activity-bridge` (local plugin)
